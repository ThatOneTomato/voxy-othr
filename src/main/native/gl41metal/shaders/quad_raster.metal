...
static inline float precompute_light_packed(uint lightRaw) {
  float2 lightBase =
      float2(float((lightRaw >> 4u) & 0xfu), float(lightRaw & 0xfu)) / 15.0f;
...
static inline float2 precompute_tex_origin(uint face, uint modelId) {
  float2 faceBase = float2(float(face >> 1u) / (3.0f * 256.0f),
                           float(face & 1u) / (2.0f * 256.0f));
  float2 modelBase = float2(float(modelId & 0xffu) / 256.0f,
                            float((modelId >> 8u) & 0xffu) / 256.0f);
...
static inline float4 unpack_rgba(uint colour) {
  return float4(float((colour >> 24u) & 0xffu), float((colour >> 16u) & 0xffu),
                float((colour >> 8u) & 0xffu), float(colour & 0xffu)) /
         255.0f;
}
...
// Packed to 44 bytes (from 68) for mesh shader output budget. All
// per-quad-constant fields are packed into uints; only `uv` needs
// perspective-correct interpolation. Fragment shaders unpack.
//
//   flagsFaceCov layout:
//     bits  0-15: flags (bit0=discard, bits2-3=tintState, bit6=isShaded,
//     bits8-11=qSizeX, bits12-15=qSizeY) bits 16-18: face (0-7) bit     19:
//     coverage (0=skip, 1=active fragment)
//
//   colorTintPacked: raw RGBA8 tint colour (0xffffffff = no tint); unpack with
//   unpack_rgba().yzwx lightPacked: raw 8-bit lightId (sky<<4 | block); unpack
//   to lightMap float2 in fragment
...
// === GL41Metal distant gbuffer ABI: 3 shared IOSurface textures + 1 private
// depth ===
//
// WHY ONLY THREE TEXTURES: Apple's OpenGL 4.1 driver caps a fragment program at
// GL_MAX_TEXTURE_IMAGE_UNITS = 16 samplers AND its GLSL compiler SIGSEGVs
// (inside glpLLVMCGFindSamplersAndBuffers / glLinkProgram) when a program
// actually uses all 16. The Iris bridge COLOUR program that samples this
// gbuffer also binds every shader-pack sampler (Complementary needs 12:
// gaux2/gaux4/colortex18/colortex19/vxDepthTexOpaque/vxDepthTexTrans/
// tex/depthtex1/noisetex/shadowtex0/shadowtex1/shadowcolor0). 12 pack + 4
// reconstruction = 16 crashed; folding the reconstruction to 3 samplers keeps
// the program at 12 + 3 = 15 (<= 15).
//
// The previous BGRA8 albedo target is removed: albedo is 8-bit anyway, so it is
// packed into a float channel exactly like tint, at identical precision. The 12
// RGBA32F channels below hold every field losslessly (this is the minimum: the
// data needs 12 channels).
//
// THIS STRUCT IS THE AUTHORITATIVE LAYOUT. The GL side (GlDistantTerrainBridge
// GLSL_GBUFFER_DECODE) decodes the exact same bit layout, shared_gbuffer.mm
// allocates the matching IOSurface formats, and traversal_pipeline.mm clears
// the three attachments. Changing a field here means changing all three.
//
// LOSSLESSNESS RULES (do not break these):
//  * Integers are stored as exact float *values* (a 32-bit float represents
//  every integer
//    in [0, 2^24] exactly). Every packed integer below stays < 2^24. We never
//    use as_type<float>(uint) bit reinterpretation: a NaN/Inf bit pattern can
//    be canonicalised by the GPU on store and silently corrupt the bits.
//  * The GL side reads with NEAREST filtering on RGBA32F, so each stored float
//  value is
//    returned bit-exact and uint(value) recovers the integer without a +0.5
//    nudge (a +0.5 would overflow the 2^24 exact range for the 24-bit packed
//    fields).
//  * albedo/tint are 8-bit per channel (same precision as the old BGRA8 albedo
//  target).
//    lightMap packs its two coords at 12-bit each: the source light levels are
//    only 16 discrete steps per axis, so 12 bits represent them exactly.
//  * Only RGBA32F is used; it is already validated for IOSurface<->GL interop.
//
//   gbuffer0 RGBA32F  .x = atlas uv.x  .y = atlas uv.y  .z = quad tile.x  .w =
//   quad tile.y gbuffer1 RGBA32F  .x = ndc depth [0,1]  .y = modelId (< 2^16)
...
//                     .z = tintPacked   = (tr<<16)|(tg<<8)|tb (each 0..255;
//                     tint alpha is opaque
//                          here, a future translucent gbuffer carries it
//                          separately)
//                     .w = (ao8<<12)|(face<<9)|(flags<<1)|coverage
//                          (face 0..7, flags 0..255, coverage 0/1; coverage
//                          doubles as the "fragment ran" flag, 0 on cleared
//                          pixels. ao8 0..255 is the SSAO factor baked by the
//                          ssao.metal pass AFTER this raster; this fragment
//                          always writes ao8 == 0, meaning "no AO data", and
//                          the GL side falls back to 1.0. Max value < 2^20,
//                          still float-exact.)
...
static inline float3 apply_world_curvature(float3 point,
                                           constant SceneUniform& scene) {
...
fragment QuadFragmentOut voxy_quad_fragment(QuadVertexOut in [[stage_in]],
  ...
  constexpr sampler atlasSampler(coord::normalized, address::clamp_to_edge,
                                 filter::nearest, mip_filter::linear);
  float2 tile = floor(in.uv);
  float2 uvFrac = in.uv - tile;
  float2 texPos = in.texOrigin + uvFrac / float2(3.0f * 256.0f, 2.0f * 256.0f);
  float2 atlasTexel =
      0.5f / float2(float(atlas.get_width()), float(atlas.get_height()));
  float2 faceExtent = 1.0f / float2(3.0f * 256.0f, 2.0f * 256.0f);
  texPos = clamp(texPos, in.texOrigin + atlasTexel,
                 in.texOrigin + faceExtent - atlasTexel);
  ...
  float2 quadSize =
      float2(float((flags >> 8u) & 0xfu), float((flags >> 12u) & 0xfu));
  ...
  if (tintState == 2u) {
    ...
    if (abs(sampled.r - sampled.g) < 0.02f &&
        abs(sampled.g - sampled.b) < 0.02f) {
      if (in.colorTintPacked != 0xffffffffu) {
        tint = unpack_rgba(in.colorTintPacked).yzwx;
      }
    }
  ...
  float albedoPacked = float(
      pack_float_to_unorm4x8(float4(sampled.b, sampled.g, sampled.r, 0.0f)));
  float tintPacked =
      float(pack_float_to_unorm4x8(float4(tint.b, tint.g, tint.r, 0.0f)));
  float faceFlagsCoverage =
      float((normalFace << 9u) | ((flags & 0xFFu) << 1u) | 1u);

  QuadFragmentOut out;
  out.gbuffer0 = float4(texPos.x, texPos.y, tile.x, tile.y);
  out.gbuffer1 = float4(in.position.z, in.gbuf1_y, in.gbuf1_z, in.gbuf1_w);
  out.gbuffer2 =
      float4(albedoPacked, in.gbuf2_y, tintPacked, faceFlagsCoverage);
  return out;
}
...
// ============================
//
// Replaces the indexed vertex pipeline for opaque raster. The object shader
// reads the precomputed buffer in batches of MESH_QUADS_PER_TG quads and
// forwards each batch via the object_data payload to one mesh TG that emits the
// geometry directly.

// Mesh batch size: configurable via Metal function constants at pipeline
// creation. Benchmark candidates: 16, 32, 64. Budget per mesh TG at each:
...
constant uint MESH_QUADS_PER_TG =
    is_function_constant_defined(MESH_BATCH_SIZE_FC) ? MESH_BATCH_SIZE_FC : 32u;
...
kernel void prepare_mesh_args(device const uint* worklistCounter [[buffer(0)]],
...
// Object shader payload: section-level metadata so mesh TGs can resolve quads
// inline. Each visible face group is recorded as (geometryOffset, count) in the
// groups array.
...
// 1 object TG per WorkItem. Iterates face groups, applies camera-facing
// culling, builds the payload with per-group geometry offsets, and dispatches
// mesh TGs to cover all visible quads. This replaces the old prepare_quad_draw
// compute pass for opaque.
[[object, max_total_threadgroups_per_mesh_grid(1024)]]
void voxy_opaque_object(device const WorkItem* worklist [[buffer(0)]],
  ...
  int3 baseSection =
      (sectionPos * int(1u << lodLevel)) - scene.baseSectionFrame.xyz;
  ...
  if (threadIdx == 0u) {
    ...
    uint meshTGs =
        min((visibleQuads + MESH_QUADS_PER_TG - 1u) / MESH_QUADS_PER_TG, 1024u);
    meshGrid.set_threadgroups_per_grid(uint3(meshTGs, 1u, 1u));
  }
}

using OpaqueMesh = metal::mesh<QuadVertexOut, void, MESH_MAX_VERTS,
                               MESH_MAX_PRIMS, metal::topology::triangle>;
...
// Corner 0 of each quad reads device memory; all 4 corners read resolved data
// from threadgroup shared memory after a barrier, reducing scattered device
// reads by 75%.
[[mesh, max_total_threads_per_threadgroup(MESH_MAX_VERTS)]]
void voxy_opaque_mesh(const object_data OpaquePayload& payload [[payload]],
    ...
    if (quad != 0ul) {
      ...
      if (modelId < (1u << 16u) && modelPresent[modelId] != 0u) {
        ...
        if (faceData != 0xffffffffu) {
          ...
          float3 qStart =
              float3(qStartRaw) +
              swizzle_axis(face >> 1u,
                           float3(fSize.x, fSize.z,
                                  mix(depthOffset, 1.0f - depthOffset,
                                      float(face & 1u))));
          ...
          if (tintState != 0u) {
            ...
            if ((model.flagsA & 2u) != 0u) {
              tintPacked =
                  modelColours[model.colourTint + extract_biome_id(quad)];
            }
          }
          ...
          sq.uvBase = fSize.xz;
          sq.face = face;
          sq.flagsFaceCov = (uint(useDiscard) | (tintState << 2u) |
  ...
  if ((sq.flagsFaceCov & (1u << 19u)) != 0u) {
    float2 cornerMask =
        float2(float((corner >> 1u) & 1u), float(corner & 1u)) * sq.lodScale;
    float3 point =
        float3(sq.basePoint) +
        swizzle_axis(sq.face >> 1u,
    ...
    vert.uv = float2(sq.uvBase) +
              float2(sq.quadSizeAdd) *
...
// ============================
//
// Distance sort: mirrors GL46 buildtranslucents.comp. Translucent sections are
// binned by Manhattan distance into TRANSLUCENT_BUCKET_COUNT buckets (nearer
// sections -> higher bucket), the per-bucket quad counts are
// exclusive-prefix-summed into start offsets, then each section's group-0 quads
// are scattered into its bucket range. The translucent raster then draws bucket
// 0..N ascending, i.e. far -> near, so the Metal over-blend into tgbufferAccum
// accumulates in the correct order.

constant uint TRANSLUCENT_BUCKET_COUNT = 1024u;

// Count pass: bin each translucent work item by section distance and accumulate
// its quad count into the bucket. Caches the bucket in WorkItem.reserved so the
// scatter pass does not recompute it.
kernel void prepare_translucent_sort(device WorkItem* worklist [[buffer(0)]],
                                     device const uint* worklistCounter
                                     [[buffer(1)]],
                                     device const SectionMeta* sections
                                     [[buffer(2)]],
                                     device atomic_uint* distanceBuckets
                                     [[buffer(3)]],
                                     constant SceneUniform& scene [[buffer(4)]],
                                     uint gid [[thread_position_in_grid]]) {
  ...
  uint bucket = (TRANSLUCENT_BUCKET_COUNT - 1u) -
                min(dist, TRANSLUCENT_BUCKET_COUNT - 1u);
  uint quadCount = item.lodAndQuadCount & 0x00ffffffu;
  atomic_fetch_add_explicit(&distanceBuckets[bucket], quadCount,
                            memory_order_relaxed);
  worklist[gid].reserved = bucket;
}

// Exclusive prefix sum over the buckets (single thread; 1024 elements run once
// per frame). After this distanceBuckets[b] is the start quad offset for bucket
// b, clamped to the raster capacity; it then doubles as the running cursor
// consumed by scatter_translucent_draw. Also fills drawArgs.
kernel void prefix_sum_translucent(device uint* distanceBuckets [[buffer(0)]],
                                   device const uint* worklistCounter
                                   [[buffer(1)]],
...
// Scatter + expand: place each section's group-0 quads into its bucket range
// (back->front order).
kernel void scatter_translucent_draw(
  ...
  SectionMeta meta = sections[item.meshId];
  // group 0 occupies [meta.a.w, meta.a.w + group_count(meta, 0)) - it is the
  // first group, before every opaque group (prepare_quad_draw advances
  // sourceOffset past it without emitting it).
  uint sourceOffset = meta.a.w;
  uint cap = scene.rasterLimits.x;
  uint base = atomic_fetch_add_explicit(&distanceBuckets[bucket], quadCount,
                                        memory_order_relaxed);
...
// Translucent mesh shader pair. Reads sorted QuadDrawRefs directly and resolves
// quad data inline, eliminating the translucent precompute_quads pass. Object
// grid = ceil(totalSortedQuads / MESH_QUADS_PER_TG); grid order preserves the
// far->near draw order.
...
void voxy_translucent_object(device const uint* drawArgs [[buffer(0)]],
                             uint objectId [[threadgroup_position_in_grid]],
                             uint threadIdx [[thread_index_in_threadgroup]],
                             object_data TranslucentBatchPayload& payload
                             [[payload]],
                             mesh_grid_properties meshGrid) {
...
using TranslucentMesh = metal::mesh<QuadVertexOut, void, MESH_MAX_VERTS,
                                    MESH_MAX_PRIMS, metal::topology::triangle>;

[[mesh, max_total_threads_per_threadgroup(MESH_MAX_VERTS)]]
void voxy_translucent_mesh(const object_data TranslucentBatchPayload& payload
                           [[payload]],
    ...
    if (quad != 0ul) {
      ...
      if (modelId < (1u << 16u) && modelPresent[modelId] != 0u) {
        ...
        if (faceData != 0xffffffffu) {
          ...
          sq.uvBase = fSize.xz;
          sq.face = face;
          sq.flagsFaceCov = (((faceData >> 22u) & 1u) | (tintState << 2u) |
                             (((model.flagsA >> 3u) & 1u) << 6u) |
                             ((qSize.x - 1u) << 8u) | ((qSize.y - 1u) << 12u)) |
                            (face << 16u) | (1u << 19u);
          sq.colorTintPacked = tintPacked;
...
// Translucent distant gbuffer ABI (see SharedDistantGbuffer /
// GlDistantTerrainBridge). Three RGBA32F shared targets. tgbuffer0/1 hold the
// FRONT-most translucent surface (blend off; nearest wins via the
// framebuffer-fetch depth compare below, since within-section draw order is
// arbitrary) for strict Iris water-program shading; tgbufferAccum holds the
// order-independent accumulation of ALL layers (rgb = additive sum of
// premultiplied flat colours, alpha = 1 - prod(1-a_i)) so layers behind the
// front surface still contribute colour in the deferred-hybrid composite.
//
//   tgbuffer0 RGBA32F  .x albedoPacked (r<<16|g<<8|b)  .y lightPacked
//   (lx12<<12|ly12)
//                      .z tintPacked (tr<<16|tg<<8|tb) .w
//                      (face<<9)|(flags<<1)|coverage
//   tgbuffer1 RGBA32F  .x ndc depth [0,1]  .y alpha[0,1]  .z customId&0xFFFFFF
//   .w customId>>24 tgbufferAccum RGBA32F  .xyz premultiplied
//   (albedo*tint*alpha)  .w alpha   (over-blended)
...
// prevT0/prevT1 are framebuffer-fetch reads (Apple-silicon tile memory) of the
// current tgbuffer0/1 contents. Draw order is only sorted per-SECTION
// (back->front across sections, arbitrary within one section), so "last write
// wins" does NOT select the nearest surface for layers inside the same section
// (e.g. a glass box standing in water). Instead each fragment compares its
// depth against the stored front surface and keeps whichever is nearer, making
// the front-surface extraction order-independent.
fragment TranslucentFragmentOut voxy_translucent_fragment(
    QuadVertexOut in [[stage_in]], bool isFrontFacing [[front_facing]],
    float4 prevT0 [[color(0)]], float4 prevT1 [[color(1)]],
  ...
  // Single-sided translucency, aligned to the quad's SEMANTIC face rather than
  // raw winding. The mesher emits the same corner order for both directions of
  // a face pair (that is why the opaque fragment derives the observed face from
  // [[front_facing]]), so fixed-function MTLCullModeFront (added to stop glass
  // rendering double-layered) also culled the exposed side of +Z/+X/-Y faces -
  // distant water and stained glass lost their side walls. Instead keep only
  // fragments seen from the direction the stored face points to, which is
  // exactly vanilla's GL_CULL_FACE/GL_BACK behaviour for translucent blocks.
  bool glFrontFacing = !isFrontFacing;
  bool faceAxisNonZero = (face >> 1u) != 0u;
  bool viewedFromBehind =
      (bool(face & 1u)) != (glFrontFacing != faceAxisNonZero);
  if (viewedFromBehind) {
    discard_fragment();
  }
  uint normalFace = face;
  ...
  // Fragments viewed from behind the stored face were discarded above, so the
  // observed face is always the stored face here (no winding-derived flip
  // needed).
  float albedoPacked = float(
      pack_float_to_unorm4x8(float4(sampled.b, sampled.g, sampled.r, 0.0f)));
  float tintPacked =
  ...
  if (isShaded) {
    ...
    if (fAxis == 1u)
      ...
      faceShade = 0.6f;
    else if (normalFace != 1u)
      faceShade = 0.5f;
  }

  float3 flatColour =
      saturate(sampled.rgb) * saturate(tint.rgb) * faceShade * alpha;

  TranslucentFragmentOut out;
  // Nearest-wins front surface: keep the stored fragment when it has coverage
  // (tgbuffer0.w low bit) and is closer than this one; tgbuffer0/1 blending is
  // off, so the returned value is stored verbatim either way. tgbufferAccum
  // always accumulates (order-independent blend).
  ...
  if (prevCovered && prevT1.x <= screenDepth) {
    ...
    out.tgbuffer0 =
        float4(albedoPacked, in.gbuf2_y, tintPacked, faceFlagsCoverage);
    out.tgbuffer1 =
        float4(screenDepth, saturate(alpha), in.gbuf1_z, in.gbuf1_w);
  }
