...
bool createSlotFrameResources(Slot* slot, NativeContext* context, TerrainResources* terrain,
  ...
  frame->sceneUniform = [context->device newBufferWithLength:SCENE_UNIFORM_BYTES
                                                     options:MTLResourceStorageModeShared];
  frame->drawArgs = [context->device newBufferWithLength:5 * sizeof(uint32_t)
                                                 options:MTLResourceStorageModeShared];
  ...
  frame->translucentWorklist = [context->device
      newBufferWithLength:static_cast<NSUInteger>(terrain->maxWorklistItems) * WORKLIST_ITEM_BYTES
                  options:MTLResourceStorageModeShared];
  frame->translucentQuadRefs = [context->device
      newBufferWithLength:static_cast<NSUInteger>(terrain->maxRasterQuads) * QUAD_DRAW_REF_BYTES
                  options:MTLResourceStorageModeShared];
  frame->translucentDrawArgs = [context->device newBufferWithLength:5 * sizeof(uint32_t)
                                                            options:MTLResourceStorageModeShared];
  frame->translucentDistanceBuckets =
      [context->device newBufferWithLength:TRANSLUCENT_BUCKET_COUNT * sizeof(uint32_t)
                                   options:MTLResourceStorageModeShared];
  frame->meshIndirectArgs = [context->device newBufferWithLength:3 * sizeof(uint32_t)
                                                         options:MTLResourceStorageModeShared];
  ...
  if (frame->queueMeta == nil || frame->scratchQueueA == nil || frame->scratchQueueB == nil ||
      ...
      frame->traversalStats == nil || frame->sceneUniform == nil || frame->drawArgs == nil ||
      ...
      frame->translucentDistanceBuckets == nil || frame->meshIndirectArgs == nil ||
      frame->translucentMeshIndirectArgs == nil) {
...
void clearSlotFrameResources(FrameResources* frame, TerrainResources* terrain) {
  ...
  std::memset([frame->translucentWorklist contents], 0,
              static_cast<size_t>(terrain->maxWorklistItems) * WORKLIST_ITEM_BYTES);
  std::memset([frame->translucentQuadRefs contents], 0,
...
void updateSectionResidencyFromMetadata(TerrainResources* terrain, int sectionId,
                                        const uint32_t* meta) {
...
Java_me_cortex_voxy_client_core_rendering_backend_gl41metal_jni_NativeBindings_createTerrainResources(
    JNIEnv* env, jclass, jlong handle, jint maxSections, jlong geometryCapacityBytes, jint maxNodes,
    jint maxTraversalQueue, jint maxTraversalRequests, jint maxWorklistItems, jint maxRasterQuads,
    jint atlasWidth, jint atlasHeight, jint atlasMipLevels, jint meshBatchSize) {
  @autoreleasepool {
    ...
    if (maxSections <= 0 || geometryCapacityBytes <= 0 || maxNodes <= 0 || maxTraversalQueue <= 0 ||
        maxTraversalRequests <= 0 || maxWorklistItems <= 0 || maxRasterQuads <= 0 ||
    ...
    terrain->topNodeBuffer = [context->device
        newBufferWithLength:static_cast<NSUInteger>(maxTraversalQueue) * sizeof(uint32_t)
                    options:MTLResourceStorageModeShared];
    terrain->modelBuffer = [context->device newBufferWithLength:MODEL_COUNT * MODEL_SIZE
                                                        options:MTLResourceStorageModeShared];
    terrain->modelColourBuffer = [context->device newBufferWithLength:MODEL_COUNT * sizeof(uint32_t)
                                                              options:MTLResourceStorageModeShared];
    terrain->modelPresentBuffer =
        [context->device newBufferWithLength:MODEL_COUNT * sizeof(uint32_t)
                                     options:MTLResourceStorageModeShared];
    terrain->validationStats = [context->device newBufferWithLength:6 * sizeof(uint32_t)
                                                            options:MTLResourceStorageModeShared];
...
Java_me_cortex_voxy_client_core_rendering_backend_gl41metal_jni_NativeBindings_uploadModel(
    JNIEnv* env, jclass, jlong handle, jint modelId, jlong modelAddress, jlong modelBytes,
    jlong textureAddress, jlong textureBytes, jint, jint) {
  @autoreleasepool {
    NativeContext* context = requireContext(env, handle);
    if (context == nullptr || context->terrain == nullptr) {
      return;
    }
    if (modelId < 0 || modelId >= static_cast<jint>(MODEL_COUNT) ||
        modelBytes < static_cast<jlong>(MODEL_SIZE)) {
      throwJava(env, "GL41Metal uploadModel received invalid model data");
      return;
    }
    TerrainResources* terrain = context->terrain.get();
    std::memcpy(static_cast<uint8_t*>([terrain->modelBuffer contents]) +
                    static_cast<uint64_t>(modelId) * MODEL_SIZE,
                reinterpret_cast<const void*>(modelAddress), MODEL_SIZE);
    reinterpret_cast<uint32_t*>([terrain->modelPresentBuffer contents])[modelId] = 1;

    if (terrain->atlas != nil) {
      if (textureAddress == 0 || textureBytes <= 0) {
        throwJava(env,
                  "GL41Metal uploadModel requires texture data when atlas uploads are enabled");
        return;
      }
      ...
      for (int mip = 0; mip < terrain->atlasMipLevels; mip++) {
        ...
        NSUInteger bytesPerRow =
            static_cast<NSUInteger>(alignUp(static_cast<uint64_t>(width) * 4, 256));
        int mipAtlasHeight = terrain->atlasHeight >> mip;
        int mipAtlasX = atlasX >> mip;
        int mipAtlasY = atlasY >> mip;
        mipUploads.push_back(
            AtlasMipUpload{textureOffset, stagingBytes, bytesPerRow, static_cast<NSUInteger>(width),
                           static_cast<NSUInteger>(height), static_cast<NSUInteger>(mipAtlasX),
      ...
      for (const AtlasMipUpload& upload : mipUploads) {
        ...
        for (NSUInteger row = 0; row < upload.height; row++) {
          std::memcpy(mipDest + (upload.height - row - 1) * upload.bytesPerRow,
...
Java_me_cortex_voxy_client_core_rendering_backend_gl41metal_jni_NativeBindings_uploadBiomeData(
    JNIEnv* env, jclass, jlong handle, jlong colourAddress, jlong colourBytes,
    jlong modelBiomePairsAddress, jlong modelBiomePairsBytes) {
  NativeContext* context = requireContext(env, handle);
  if (context == nullptr || context->terrain == nullptr) {
    return;
  }
  ...
  std::memcpy(static_cast<uint8_t*>([terrain->modelColourBuffer contents]) +
                  static_cast<uint64_t>(minBiomeBase) * sizeof(uint32_t),
              reinterpret_cast<const void*>(colourAddress), static_cast<size_t>(colourBytes));
  ...
  for (uint64_t i = 0; i < pairCount; i++) {
    ...
    if (modelId < MODEL_COUNT) {
      std::memcpy(modelBase + static_cast<uint64_t>(modelId) * MODEL_SIZE + 28, &biomeBase,
                  sizeof(uint32_t));
    }
  }
...
Java_me_cortex_voxy_client_core_rendering_backend_gl41metal_jni_NativeBindings_uploadSection(
    JNIEnv* env, jclass, jlong handle, jint sectionId, jlong sectionPos, jint aabb,
    jint childExistence, jlong offsetsAddress, jlong offsetsBytes, jlong geometryAddress,
    jlong geometryBytes) {
  NativeContext* context = requireContext(env, handle);
  if (context == nullptr || context->terrain == nullptr) {
    return;
  ...
  if (sectionId < 0 || sectionId >= terrain->maxSections ||
      offsetsBytes < 8 * static_cast<jlong>(sizeof(uint32_t)) || geometryBytes <= 0 ||
      (geometryBytes & 7) != 0) {
  ...
  std::memcpy(static_cast<uint8_t*>([terrain->geometry contents]) + geometryOffset,
              reinterpret_cast<const void*>(geometryAddress), static_cast<size_t>(geometrySize));
...
Java_me_cortex_voxy_client_core_rendering_backend_gl41metal_jni_NativeBindings_uploadGeometry(
    JNIEnv* env, jclass, jlong handle, jint geometryElementOffset, jlong geometryAddress,
    jlong geometryBytes) {
  NativeContext* context = requireContext(env, handle);
  if (context == nullptr || context->terrain == nullptr) {
    return;
  ...
  TerrainResources* terrain = context->terrain.get();
  if (geometryElementOffset < 0 || geometryAddress == 0 || geometryBytes < 0 ||
      (geometryBytes & 7) != 0) {
  ...
  if (byteOffset > terrain->geometryCapacityBytes ||
      bytes > terrain->geometryCapacityBytes - byteOffset) {
