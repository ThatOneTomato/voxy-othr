...
import me.cortex.voxy.client.core.rendering.backend.gl46.pipeline.IrisVoxyRenderPipeline;
...
public class IrisVoxyRenderPipelineData {
  ...
  public static IrisVoxyRenderPipelineData buildPipeline(
    ...
    var opaqueDrawTargets =
        getDrawBuffers(
            patch.getOpqaueTargets(),
            ipipe.getFlippedAfterPrepare(),
            ((IrisRenderingPipelineAccessor) ipipe).getRenderTargets());
    var translucentDrawTargets =
        getDrawBuffers(
            patch.getTranslucentTargets(),
            ipipe.getFlippedAfterPrepare(),
            ((IrisRenderingPipelineAccessor) ipipe).getRenderTargets());

    // TODO: need to transform the string patch with the uniform decleration aswell as sampler
    // declerations
  ...
  private static int[] getDrawBuffers(
      int[] targets, ImmutableSet<Integer> stageWritesToAlt, RenderTargets rt) {
    int[] targetTextures = new int[targets.length];
    for (int i = 0; i < targets.length; i++) {
      RenderTarget target = rt.getOrCreate(targets[i]);
      int textureId =
          stageWritesToAlt.contains(targets[i]) ? target.getAltTexture() : target.getMainTexture();
  ...
  private static StructLayout createUniformLayoutStructAndUpdater(
    ...
    // Emit the ordering, note this is not optimial, but good enough, e.g. if have even number of
    // align 2, emit that after align 4
    int pos = 0;
  ...
  private static List<UniformWritingHolder> createUniformSet(
      CustomUniforms cu, IrisShaderPatch patch) {
    ...
    DynamicLocationalUniformHolder uniformBuilder =
        new DynamicLocationalUniformHolder() {
          ...
          public OptionalInt location(String uniformName, UniformType uniformType) {
            // Yes am aware how performant inefficent this is... just dont care tbh since is on
            // setup and is small
            var names = patch.getUniformList();
            for (int i = 0; i < names.length; i++) {
            ...
            entry -> {
              if (!seenUniforms.add(entry.getKey().getName())) {
                throw new IllegalArgumentException(
                    "Already added uniform: " + entry.getKey().getName());
              }
              uniforms.add(
    ...
    if (uniforms.size() != patch.getUniformList().length) {
      ...
      Logger.error(
          "The following uniforms could not be found: ["
  ...
  private static ImageSet createImageSet(IrisRenderingPipeline ipipe, IrisShaderPatch patch) {
    ...
    SamplerHolder samplerBuilder =
        new SamplerHolder() {
          ...
          public boolean addDynamicSampler(
            ...
            samplerSet.add(
                new TextureWSampler(
                    this.name(names), texture, sampler != null ? sampler.getId() : -1));
            return true;
          }
          ...
          public void addExternalSampler(int texture, String... names) {
            ...
            if (ex != null) {
              samplerSet.add(
                  new TextureWSampler(
                      name, ex, 0)); // unbind any sampler and use the externalTextureSupplier
            } else {
    ...
    ImageHolder imageBuilder =
        new ImageHolder() {
          ...
          public void addTextureImage(
              IntSupplier intSupplier, InternalTextureFormat internalTextureFormat, String s) {}
        };

    ipipe.addGbufferOrShadowSamplers(
        samplerBuilder, imageBuilder, ipipe::getFlippedAfterPrepare, false, true, true, false);
    ...
    for (var entry : samplerSet) {
      ...
      builder
          .append("layout(binding=(BASE_SAMPLER_BINDING_INDEX+")
          .append(i)
          .append(")) uniform ")
          .append(samplerType)
          .append(" ")
