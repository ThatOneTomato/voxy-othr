...
public class VoxyUniforms {
  ...
  public static Matrix4f
      getViewProjection() { // This is 1 frame late ;-; cries, since the update occurs _before_ the
  ...
  public static Matrix4f
      getModelView() { // This is 1 frame late ;-; cries, since the update occurs _before_ the voxy
    // render pipeline
    var getVrs = (VoxyRenderSystemAccess) Minecraft.getInstance().levelRenderer;
    if (getVrs == null || getVrs.voxy$getRenderSystem() == null) {
      return new Matrix4f();
  ...
  public static Matrix4f
      getProjection() { // This is 1 frame late ;-; cries, since the update occurs _before_ the voxy
    // render pipeline
    var getVrs = (VoxyRenderSystemAccess) Minecraft.getInstance().levelRenderer;
    if (getVrs == null || getVrs.voxy$getRenderSystem() == null) {
      return new Matrix4f();
  ...
  public static void addUniforms(UniformHolder uniforms) {
    uniforms
        .uniform1i(
            PER_FRAME,
            "vxRenderDistance",
            () -> Math.round(VoxyConfig.CONFIG.sectionRenderDistance * 32)) // In chunks
        ...
        .uniformMatrix(
            PER_FRAME, "vxViewProjPrev", new PreviousMat(VoxyUniforms::getViewProjection))
    ...
    if (IrisShaderPatch.IMPERSONATE_DISTANT_HORIZONS) {
      uniforms
          ...
          .uniform1i(
              PER_FRAME,
              "dhRenderDistance",
              () -> Math.round(VoxyConfig.CONFIG.sectionRenderDistance * 32 * 16)) // In blocks
          .uniformMatrix(PER_FRAME, "dhProjection", VoxyUniforms::getProjection)
          .uniformMatrix(
              PER_FRAME, "dhProjectionInverse", new Inverted(VoxyUniforms::getProjection))
          .uniformMatrix(
              PER_FRAME, "dhPreviousProjection", new PreviousMat(VoxyUniforms::getProjection));
    }
  }
