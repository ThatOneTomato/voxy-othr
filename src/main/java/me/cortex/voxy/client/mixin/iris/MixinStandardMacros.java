...
public abstract class MixinStandardMacros {
  ...
  private static final boolean SUPPRESS_VOXY_DEFINE =
      Boolean.parseBoolean(System.getProperty("voxy.disableVoxyDefine", "false"));
  ...
  private static ImmutableList<StringPair> voxy$injectVoxyDefine(
      Collection<StringPair> list, Operation<ImmutableList<StringPair>> original) {
    if (VoxyConfig.CONFIG.isRenderingEnabled()
        && IrisUtil.SHADER_SUPPORT
        && !SUPPRESS_VOXY_DEFINE) {
      define(
          (List<StringPair>) list, "VOXY", Integer.toString(IrisShaderPatch.SHADER_DEFINE_VERSION));
