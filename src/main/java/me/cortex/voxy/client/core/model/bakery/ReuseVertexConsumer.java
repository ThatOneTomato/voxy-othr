...
public final class ReuseVertexConsumer implements VertexConsumer {
  ...
  public ReuseVertexConsumer quad(BakedQuad quad, int metadata) {
    ...
    for (int i = 0; i < 4; i++) {
      ...
      int j = i * 8;
      this.addVertex(
          Float.intBitsToFloat(vertices[j]),
          Float.intBitsToFloat(vertices[j + 1]),
          Float.intBitsToFloat(vertices[j + 2]));
  ...
  private void ensureCanPut() {
    ...
    var newBuffer =
        new MemoryBuffer(
            (((int) (this.buffer.size * 2) + VERTEX_FORMAT_SIZE - 1) / VERTEX_FORMAT_SIZE)
                * VERTEX_FORMAT_SIZE);
  ...
  public ReuseVertexConsumer reset() {
    ...
    this.ptr =
        this.buffer.address
            - VERTEX_FORMAT_SIZE; // the thing is first time this gets incremented by FORMAT_STRIDE
    return this;
  }
