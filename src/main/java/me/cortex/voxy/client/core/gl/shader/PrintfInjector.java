...
public class PrintfInjector implements ShaderProcessor {
  ...
  public PrintfInjector(
      int bufferSize, int bufferBindingIndex, Consumer<String> callback, Runnable pre) {
      ...
      subCode.append(
          String.format(
              "{"
                  ...
                  + "printfOutputStruct.stream[printfWriteIndex]=%s;",
              types.size() + 1,
              this.printfStringMap.computeIfAbsent(
                  fmtStr,
                  a -> {
  ...
  public void download() {
    ...
    nglClearNamedBufferSubData(
        this.textBuffer.id, GL_R32UI, 0, 4, GL_RED_INTEGER, GL_UNSIGNED_INT, 0);
  }
