...
public class UploadStream {
  public static final int BASE_ALLOCATION_ALIGNEMENT =
      Math.max(Capabilities.INSTANCE.ssboBindingAlignment, 16);
  ...
  public void upload(
      GlBuffer buffer,
      long destOffset,
      MemoryBuffer data) { // Note: does not free data, nor does it commit
  ...
  public long rawUploadAddress(int size) {
    ...
    // Force natural size alignment, this should ensure that _all_ allocations are aligned to this
    // size, note, this only effects the allocation block
    ...
    if (this.caddr == -1 || !this.allocationArena.expand(this.caddr, (int) size)) {
      ...
      if (this.caddr == SIZE_LIMIT) {
        ...
        Logger.error(
            "Upload stream full, preemptively committing, this could cause bad things to happen");
        int attempts = 10;
  ...
  public void commit() {
    ...
    glMemoryBarrier(
        GL_BUFFER_UPDATE_BARRIER_BIT); // |GL_SHADER_STORAGE_BARRIER_BIT|GL_UNIFORM_BARRIER_BIT
  ...
  private void tick(boolean commit) {
    ...
    while (!this.frames.isEmpty()) {
      // Since the ordering of frames is the ordering of the gl commands if we encounter an
      // unsignaled fence
  ...
  public static int alignUpAlloc(int val) {
    return ((val + BASE_ALLOCATION_ALIGNEMENT - 1) / BASE_ALLOCATION_ALIGNEMENT)
        * BASE_ALLOCATION_ALIGNEMENT;
  }
}
