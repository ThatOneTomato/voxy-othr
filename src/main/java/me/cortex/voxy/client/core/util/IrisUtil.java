...
public class IrisUtil {
    ...
    public static ShaderPatchBridgePayload captureShaderPatchBridgePayload(IrisRenderingPipeline pipeline) {
     ...
     */
    public static ShaderPatchBridgePayload captureTranslucentShaderPatchBridgePayload(IrisRenderingPipeline pipeline) {
        if (!irisShaderPackEnabled()) {
            return ShaderPatchBridgePayload.unavailable("Iris shader pack is not active");
        }
        if (!(pipeline instanceof IGetVoxyPatchData patchSource)) {
        ...
        if (translucentPatch == null || translucentPatch.isBlank()) {
            return ShaderPatchBridgePayload.unavailable("active shader pack provides no translucent Voxy patch");
        }
        RenderTargets renderTargets = ((IrisRenderingPipelineAccessor) pipeline).getRenderTargets();
    ...
    private static int[] resolveTargetTextures(
        ...
        // SodiumPrograms (pass == TRANSLUCENT ? getFlippedAfterTranslucent() : getFlippedAfterPrepare()).
