...
// TODO: to add remove functionallity add a "defunked" variable to the build task and set it to true
// on remove
...
public class RenderGenerationService {
  ...
  private static final class BuildTask {
    ...
    private void updatePriority(TaskPriorityMode priorityMode) {
      ...
      this.priority =
          (((lvl * 3L + Math.min(this.attempts, 3)) * 2 + this.addin) << 32)
  ...
  private final AtomicInteger holdingSectionCount =
      new AtomicInteger(); // Used to limit section holding
  ...
  public RenderGenerationService(
      WorldEngine world,
    ...
    this.service =
        sm.createService(
            () -> {
              ...
              var factory =
                  new RenderDataFactory(this.world, this.modelBakery.factory, this.emitMeshlets);
            ...
            10,
            "Section mesh generation service",
            ...
            () ->
                modelBakery.getProcessingCount() < 400
  ...
  private void computeAndRequestRequiredModels(
      IntOpenHashSet seenMissedIds, int bitMsk, long[] auxData) {
  ...
  private void computeAndRequestRequiredModels(IntOpenHashSet seenMissedIds, WorldSection section) {
    // Know this is... very much not safe, however it reduces allocation rates and other garbage, am
    // sure its "fine"
    final var factory = this.modelBakery.factory;
  ...
  private void processJob(RenderDataFactory factory, IntOpenHashSet seenMissedIds) {
    ...
    { // Remove the task from the map, this is done before we check for null sections as well the
      // task map needs to be correct
    ...
    try {
      ...
      if (task != null) {
        ...
        if (task.hasDoneModelRequestInner && task.hasDoneModelRequestOuter) {
          ...
          if (task.hasDoneModelRequestInner) {
            task.attempts++; // This is because it can be baking and just model thing isnt keeping
            // up
          }

          if (!task.hasDoneModelRequestInner) {
            // The reason for the extra id parameter is that we explicitly add/check against the
            // exception id due to e.g. requesting accross a chunk boarder wont be captured in the
            // request
            if (e.auxData
                == null) // the null check this is because for it to be, the inner must already be
              // computed
          ...
          task.addin =
              WorldEngine.getLevel(task.position) > 2
                  ? 1
                  : 0; // Single time addin which gives the models time to bake before the task
          // executes
        }

        // Keep the lock on the section, and attach it to the task, this prevents needing to
        // re-aquire it later
        if (task.section == null) {
  ...
  public void addDebugData(List<String> debug) {
    ...
    debug.add(
        "RSSQ/TFC: "
            ...
            + "/"
            + MESH_FAILED_COUNTER.get()); // render section service queue, Task Fail Counter
  }
