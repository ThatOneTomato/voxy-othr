...
public class BudgetBufferRenderer {
  private static final int VERTEX_FORMAT_SIZE = ReuseVertexConsumer.VERTEX_FORMAT_SIZE;

  private static final Shader bakeryShader =
      Shader.make()
  ...
  */
  ...
  public static void setup(long dataPtr, int quads, int texId) {
    ...
    if (immediateBuffer == null || immediateBuffer.size() < size) {
      ...
      immediateBuffer =
          new GlBuffer(size * 2L); // This also accounts for when immediateBuffer == null
