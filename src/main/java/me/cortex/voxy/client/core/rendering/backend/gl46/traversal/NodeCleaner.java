...
public class NodeCleaner {
  // TODO: use batch_visibility_set to clear visibility data when nodes are removed!! (TODO:
  // nodeManager will need to forward info to this)
  ...
  private final AutoBindingShader sorter =
      Shader.makeAuto(PrintfDebugUtil.PRINTF_processor)
          ...
          .add(ShaderType.COMPUTE, "voxy:lod/gl46/hierarchical/cleaner/sort_visibility.comp")
          .compile();

  private final AutoBindingShader resultTransformer =
      Shader.makeAuto()
          .define("OUTPUT_SIZE", OUTPUT_COUNT)
          ...
          .add(ShaderType.COMPUTE, "voxy:lod/gl46/hierarchical/cleaner/result_transformer.comp")
          .compile();

  private final AutoBindingShader batchClear =
      Shader.makeAuto()
          .define("VISIBILITY_BUFFER_BINDING", 0)
          .define("LIST_BUFFER_BINDING", 1)
          .add(ShaderType.COMPUTE, "voxy:lod/gl46/hierarchical/cleaner/batch_visibility_set.comp")
          .compile();

  final GlBuffer visibilityBuffer;
  private final GlBuffer outputBuffer =
      new GlBuffer(OUTPUT_COUNT * 4 + OUTPUT_COUNT * 8); // Scratch + output
  ...
  public void tick(GlBuffer nodeDataBuffer) {
    ...
    if (this.shouldCleanGeometry()) {
      ...
      // This should (IN THEORY naturally align its self to the pow2 max boarder, if not... well
      // undefined behavior is ok right?)
      glDispatchCompute(
          (this.nodeManager.getCurrentMaxNodeId() + (SORTING_WORKER_SIZE * WORK_PER_THREAD) - 1)
      ...
      glBindBufferRange(
          GL_SHADER_STORAGE_BUFFER, 2, this.outputBuffer.id, 4 * OUTPUT_COUNT, 8 * OUTPUT_COUNT);
      ...
      DownloadStream.INSTANCE.download(
          ...
          8 * OUTPUT_COUNT,
          buffer ->
              this.nodeManager.submitRemoveBatch(
                  buffer.copy()) // Copy into buffer and emit to node manager
          );
    }
  }
  ...
  private boolean shouldCleanGeometry() {
    if (false) {
      ...
      long remaining =
          this.nodeManager.getGeometryCapacity() - this.nodeManager.getUsedGeometryCapacity();
  ...
  public void updateIds(IntOpenHashSet collection) {
    if (!collection.isEmpty()) {
      ...
      long addr =
          UploadStream.INSTANCE.rawUploadAddress(count * 4); // Internally does upsizing alignement
      ...
      glBindBufferRange(
          ...
          addr,
          UploadStream.alignUpAlloc(count * 4));
