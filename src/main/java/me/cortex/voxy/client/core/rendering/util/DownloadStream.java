...
public class DownloadStream {
  ...
  public DownloadStream(long size) {
    this.downloadBuffer =
        new GlPersistentMappedBuffer(size, GL_MAP_READ_BIT); // |GL_MAP_COHERENT_BIT
  ...
  public void download(
      GlBuffer buffer, long downloadOffset, long size, Consumer<MemoryBuffer> consumer) {
    this.download(
        buffer,
        downloadOffset,
        size,
  ...
  public void download(
      GlBuffer buffer, long downloadOffset, long size, DownloadResultConsumer resultConsumer) {
    ...
    if (this.caddr == -1 || !this.allocationArena.expand(this.caddr, (int) size)) {
      ...
      if (this.caddr == SIZE_LIMIT) {
        Logger.warn(
            "Download stream full, preemptively committing, this could cause bad things to happen");
        this.commit();
        int attempts = 10;
  ...
  public void waitDiscard() {
    ...
    glFinish();
    while (!fence.signaled()) Thread.onSpinWait();
    fence.free();
    while (!this.frames.isEmpty()) {
      var frame = this.frames.pop();
  ...
  private record DownloadFrame(
      GlFence fence, LongArrayList allocations, ArrayList<DownloadData> data) {}
  ...
  public static final DownloadStream INSTANCE =
      new DownloadStream(1 << 25); // 32 mb download buffer
}
