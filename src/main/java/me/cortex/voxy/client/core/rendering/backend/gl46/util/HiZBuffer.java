...
public class HiZBuffer {
  private final Shader hiz =
      Shader.make()
          .add(ShaderType.VERTEX, "voxy:lod/gl46/hiz/blit.vsh")
          .add(ShaderType.FRAGMENT, "voxy:lod/gl46/hiz/blit.fsh")
          .compile()
          .name("HiZ Builder");
  ...
  private void alloc(int width, int height) {
    ...
    // We dont care about e.g. 1x1 size texture since you dont get meshlets that big to cover such a
    // large area
    // this.levels -= 1;//Arbitrary size, shinks the max level by alot and saves a significant
    // amount of processing time
    // (could probably increase it to be defined by a max meshlet coverage computation thing)

    // GL_DEPTH_COMPONENT32F //Cant use this as it does not match the depth format of the provided
    // depth buffer
  ...
  public void buildMipChain(int srcDepthTex, int width, int height) {
    if (this.width != Integer.highestOneBit(width)
        || this.height != Integer.highestOneBit(height)) {
