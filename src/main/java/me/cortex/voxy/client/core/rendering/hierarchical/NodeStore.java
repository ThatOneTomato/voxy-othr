...
public final class NodeStore {
  ...
  public void setNodeGeometry(int node, int geometryId) {
    ...
    if (geometryId > MAX_GEOMETRY_ID) {
      throw new IllegalArgumentException(
          "Geometry ptr greater than MAX_GEOMETRY_ID: " + geometryId);
    }
  ...
  public void setChildPtr(int nodeId, int ptr) {
    if (ptr >= NODE_ID_MSK || ptr < -1) {
      throw new IllegalArgumentException(
          "Node child ptr greater GEQ NODE_ID_MSK or less than -1 : " + ptr);
    }
    if (ptr == -1) {
  ...
  public void writeNode(long ptr, int nodeId) {
...

    {
      ...
      if (geometry == -2) {
        z |= 0xFFFFFF - 1; // This is a special case, which basically says to the renderer that the
        // geometry is empty (not that it doesnt exist)
      } else if (geometry == -1) {
