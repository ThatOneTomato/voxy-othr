...
public class RenderResourceReuse {
  ...
  private static long getGeometryBufferSize() {
    long geometryCapacity =
        Math.min(
                (1L << (64 - Long.numberOfLeadingZeros(Capabilities.INSTANCE.ssboMaxSize - 1)))
                    << 1,
                1L << 32)
            - 1024 /*(1L<<32)-1024*/;
    ...
    if (Capabilities.INSTANCE.isNvidia && ThreadUtils.isLinux) {
      geometryCapacity =
          Math.min(
              geometryCapacity, 2000L * 1024L * 1024L); // nvidia linux moment, force max 2gb heap
    }
    ...
    if (Capabilities.INSTANCE.canQueryGpuMemory) {
      ...
      long limit =
          Capabilities.INSTANCE.getFreeDedicatedGpuMemory()
              - (long) (1.5 * 1024 * 1024 * 1024); // 1.5gb vram buffer
