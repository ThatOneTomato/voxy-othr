...
public class RingTracker {
  ...
  public void moveCenter(int x, int z) {
    // TODO, if the new center is greater than radius from current, unload all current and load all
    // at new
    if (this.radius + 1 < Math.abs(x - this.centerX)
        || this.radius + 1 < Math.abs(z - this.centerZ)) {
      this.fillRing(false);
      this.centerX = x;
      this.centerZ = z;
      this.fillRing(true);
  ...
  private void moveZ(int delta) {
    ...
    if (delta == -1 || delta == 1) {
      for (int i = 0; i <= this.radius * 2; i++) {
        ...
        int pz =
            this.centerZ + d + delta; // Point to add (we need to offset by 1 in the mov direction)
        int nz = this.centerZ - d; // Point to rem
        if (0 < this.operations.addTo(pack(x, pz), (byte) 1)) // Load point
        throw new IllegalStateException(
              "x: " + x + ", z: " + pz + " state: " + this.operations.get(pack(x, pz)));
        if (this.operations.addTo(pack(x, nz), (byte) -1) < 0) // Unload point
        throw new IllegalStateException(
              "x: " + x + ", z: " + nz + " state: " + this.operations.get(pack(x, nz)));
      }
      this.centerZ += delta;
    } else {
  ...
  private void moveX(int delta) {
    ...
    if (delta == -1 || delta == 1) {
      for (int i = 0; i <= this.radius * 2; i++) {
        ...
        int d = this.boundDist[i] * delta;
        int px =
            this.centerX + d + delta; // Point to add (we need to offset by 1 in the mov direction)
