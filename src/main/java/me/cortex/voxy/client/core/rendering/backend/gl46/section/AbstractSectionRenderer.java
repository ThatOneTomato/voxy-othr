package me.cortex.voxy.client.core.rendering.backend.gl46.section;
...
public abstract class AbstractSectionRenderer<T extends Gl46Viewport<T>, J extends GeometryData> {
  public interface FactoryConstructor<
      VIEWPORT extends Gl46Viewport<VIEWPORT>, GEODATA extends GeometryData> {
    AbstractSectionRenderer<VIEWPORT, GEODATA> create(
        AbstractRenderPipeline pipeline, ModelStore modelStore, GEODATA geometryData);
  }

  public record Factory<VIEWPORT extends Gl46Viewport<VIEWPORT>, GEODATA extends GeometryData>(
      Class<? extends AbstractSectionRenderer<VIEWPORT, GEODATA>> clz,
      FactoryConstructor<VIEWPORT, GEODATA> constructor) {
    public AbstractSectionRenderer<VIEWPORT, GEODATA> create(
        AbstractRenderPipeline pipeline, ModelStore store, GeometryData geometryData) {
      return this.constructor.create(pipeline, store, (GEODATA) geometryData);
    }

    public static <VIEWPORT2 extends Gl46Viewport<VIEWPORT2>, GEODATA2 extends GeometryData>
        Factory<VIEWPORT2, GEODATA2> create(
      ...
      if (params.length != 3
          ...
          || !GeometryData.class.isAssignableFrom(params[2])) {
      ...
      return new Factory<>(
          ...
          (a, b, c) -> {
            try {
              return (AbstractSectionRenderer<VIEWPORT2, GEODATA2>)
                  constructor.newInstance(a, b, c);
  ...
  protected static void addDirectionalFaceTint(Shader.Builder<?> builder, ClientLevel cl) {
    ...
    builder.define(
        "Z_AXIS_FACE_TINT",
        cl.getShade(
            Direction.NORTH, true)); // assumed here that Direction.SOUTH returns the same value
    builder.define(
        "X_AXIS_FACE_TINT",
        cl.getShade(
            Direction.EAST, true)); // assumed here that Direction.WEST returns the same value
    /*
  ...
  protected static Shader tryCompilePatchedOrNormal(
      Shader.Builder<?> builder, String shader, String original) {
