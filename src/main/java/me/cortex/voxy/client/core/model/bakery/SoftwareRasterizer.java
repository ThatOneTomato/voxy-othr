...
public class SoftwareRasterizer {
  ...
  private static final long CLEAR_VALUE =
      DEPTH_MASK; // set the depth to max value and rest of bits to 0
  ...
  private void rasterTriangle() {
    ...
    // Pretty sure this is how you check for winding order aswell (if area is negative its
    // counterclockwise)
    ...
    for (int py = minY; py <= maxY; py++) {
      for (int px = minX; px <= maxX; px++) {
        ...
        if (w1 >= 0.0f && w2 >= 0.0f && w3 >= 0.0f) {
          // Dont need to worry about perspective correction afak as it should already be all
          // correct
  ...
  private void rasterPixel(int index, float b1, float b2, float b3) { // Barry coords
    ...
    if (z < 0.0f || z > 1.0f) return; // TODO: check this
    ...
    // ARBDrawBuffersBlend.glBlendFuncSeparateiARB(0, GL_ONE_MINUS_DST_ALPHA, GL_DST_ALPHA, GL_ONE,
  ...
  private void loadTransformPos(
      Matrix4f transform, long addr, int vert, Vector3f out, Vector3f otherAttributesOut) {
    ...
    if (Math.abs(this.scratch.w - 1.0f) > 0.000001f) throw new IllegalStateException();
    out.set(
        maintainPrecision(Math.fma(vec.x, 0.5f, 0.5f) * TARGET_SIZE),
        maintainPrecision(Math.fma(vec.y, 0.5f, 0.5f) * TARGET_SIZE),
        vec.z); // TODO: dont know if z transform is correct
  }
