...
import static me.cortex.voxy.client.core.rendering.backend.gl46.util.PrintfDebugUtil.PRINTF_processor;
...
public class HierarchicalOcclusionTraverser {
  public static final boolean HIERARCHICAL_SHADER_DEBUG =
  ...
  private final Int2IntOpenHashMap topNode2idxMapping =
      new Int2IntOpenHashMap(); // Used to store mapping from TLN to array index
  ...
  public HierarchicalOcclusionTraverser(
      AsyncNodeManager nodeManager, NodeCleaner nodeCleaner, RenderGenerationService meshGen) {
  ...
  public void lateStageCompile(AbstractRenderPipeline pipeline) {
    ...
    var scr = ShaderLoader.parse("voxy:lod/gl46/hierarchical/traversal_dev.comp");
    if (taa != null) {
      scr += "\n\n\n" + taa;
      this.pipeline = pipeline;
    }
    this.traversal =
        Shader.makeAuto(PRINTF_processor)
            ...
            .defineIf(
                "STATISTICS_BUFFER_BINDING", RenderStatistics.enabled, STATISTICS_BUFFER_BINDING)
  ...
  private void addTLN(int id) {
    ...
    nglClearNamedBufferSubData(
        this.topNodeIds.id, GL_R32UI, aid * 4L, 4, GL_RED_INTEGER, GL_UNSIGNED_INT, SCRATCH);
  ...
  private void remTLN(int id) {
    ...
    nglClearNamedBufferSubData(
        this.topNodeIds.id, GL_R32UI, idx * 4L, 4, GL_RED_INTEGER, GL_UNSIGNED_INT, SCRATCH);
  }

  private static void setFrustum(Gl46Viewport<?> viewport, long ptr) {
    for (int i = 0; i < 6; i++) {
  ...
  private void uploadUniform(Gl46Viewport<?> viewport) {
    ...
    final float screenspaceAreaDecreasingSize =
        VoxyConfig.CONFIG.subDivisionSize * VoxyConfig.CONFIG.subDivisionSize;
    // Screen space size for descending
    MemoryUtil.memPutFloat(
        ptr, (float) (screenspaceAreaDecreasingSize) / (viewport.width * viewport.height));
    ptr += 4;

    setFrustum(viewport, ptr);
    ptr += 4 * 4 * 6;

    MemoryUtil.memPutInt(ptr, (int) (viewport.getRenderList().size() / 4 - 1));
    ptr += 4;
    ...
    {
      final double TARGET_COUNT =
          4000; // TODO: make this configurable, or at least dynamically computed based on
      // throughput rate of mesh gen
    ...
    // Put the render distance here so that it can generate a correct circle, TODO: make it not top
    // level section sized
    MemoryUtil.memPutFloat(
        ptr, (float) Math.pow(VoxyConfig.CONFIG.sectionRenderDistance * 16 * 32, 2));
    ptr += 4;
  }

  private void bindings(Gl46Viewport<?> viewport) {
  ...
  public void doTraversal(Gl46Viewport<?> viewport) {
    ...
    nglClearNamedBufferSubData(
        viewport.getRenderList().id, GL_R32UI, 0, 4, GL_RED_INTEGER, GL_UNSIGNED_INT, 0);
  ...
  private void traverseInternal() {
    ...
    int firstDispatchSize =
        (this.topNodeCount + (1 << LOCAL_WORK_SIZE_BITS) - 1) >> LOCAL_WORK_SIZE_BITS;
    /*
    ...
    if (firstDispatchSize != 0) {
      // for some reason amd driver loves spitting out errors when its 0 (even tho it should just
      // ignore it afak) so we do it ourselves
    ...
    for (int iter = 1; iter < MAX_ITERATIONS; iter++) {
      ...
      glBindBufferBase(
          ...
          ((iter & 1) == 0 ? this.scratchQueueA : this.scratchQueueB).id);
      glBindBufferBase(
          GL_SHADER_STORAGE_BUFFER,
          NODE_QUEUE_SINK_BINDING,
          ((iter & 1) == 0 ? this.scratchQueueB : this.scratchQueueA).id);
  ...
  private void downloadResetRequestQueue() {
    ...
    nglClearNamedBufferSubData(
        this.requestBuffer.id, GL_R32UI, 0, 4, GL_RED_INTEGER, GL_UNSIGNED_INT, 0);
  }

  private void forwardDownloadResult(long ptr, long size) {
    int count = MemoryUtil.memGetInt(ptr);
    ptr += 8; // its 8 since we need to skip the second value (which is empty)
    if (count < 0 || count > 50000) {
      Logger.error(
          new IllegalStateException(
              "Count unexpected extreme value: " + count + " things may get weird"));
      return;
    }
    ...
    //    Logger.warn("Count larger than 'maxRequestCount', overflow captured. Overflowed by " +
    // (count-REQUEST_QUEUE_SIZE));
    // }
    if (count != 0) {
      this.nodeManager.submitRequestBatch(
          new MemoryBuffer(count * 8L + 8)
              .cpyFrom(ptr - 8)); // the -8 is because we incremented it by 8
    }
  }
