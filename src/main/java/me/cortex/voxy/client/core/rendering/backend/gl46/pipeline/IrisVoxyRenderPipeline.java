...
public class IrisVoxyRenderPipeline extends AbstractRenderPipeline {
  ...
  public IrisVoxyRenderPipeline(
    ...
    for (int i = 0; i < tDT.length; i++) {
      ...
      glNamedFramebufferTexture(
          this.fbTranslucent.framebuffer.id, GL30.GL_COLOR_ATTACHMENT0 + i, tDT[i], 0);
    }
    ...
    if (!this.data.skipShaderDepthHackFix) {
      this.shaderDepthHackFixTransformBlit =
  ...
  public void setupExtraModelBakeryData(ModelBakerySubsystem modelService) {
    modelService.factory.setCustomBlockStateMapping(
  ...
  protected void postOpaquePreTranslucent(Gl46Viewport<?> viewport, int sourceFrameBuffer) {
    ...
    glBlitNamedFramebuffer(
        this.fb.framebuffer.id,
        this.fbTranslucent.framebuffer.id,
        0,
        0,
        viewport.width,
        viewport.height,
  ...
  protected void finish(
    ...
    if (this.data.renderToVanillaDepth
        && srcWidth == viewport.width
        && srcHeight
            == viewport.height) { // We can only depthblit out if destination size is the same
      glColorMask(false, false, false, false);
  ...
  public void bindUniforms(int bindingPoint) {
    if (this.shaderUniforms != null) {
      GL30.glBindBufferBase(
          GL_UNIFORM_BUFFER,
          bindingPoint,
          this.shaderUniforms.id); // todo: dont randomly select this to 5
    }
  }
  ...
  private static final int UNIFORM_BINDING_POINT =
      7; // TODO make ths binding point... not randomly 5

  private StringBuilder buildGenericShaderHeader(
      AbstractSectionRenderer<?, ?> renderer, String input) {
      ...
      builder
          .append(
              "layout(binding = "
                  ...
                  + ", std140) uniform ShaderUniformBindings ")
  ...
  public String taaFunction(int uboBindingPoint, String functionName) {
    ...
    if (this.data.getUniforms() != null) {
      builder
          .append(
              "layout(binding = " + uboBindingPoint + ", std140) uniform ShaderUniformBindings ")
          .append(this.data.getUniforms().layout())
          .append(";\n\n");
    }
