...
public class Capabilities {
  ...
  public Capabilities() {
    ...
    this.ssboBindingAlignment =
        this.compute ? glGetInteger(GL_SHADER_STORAGE_BUFFER_OFFSET_ALIGNMENT) : 0;
    ...
    if (this.canQueryGpuMemory) {
      this.totalDedicatedMemory =
          glGetInteger64(GL_GPU_MEMORY_INFO_DEDICATED_VIDMEM_NVX) * 1024; // Since its in Kb
      this.totalDynamicMemory =
          (glGetInteger64(GL_GPU_MEMORY_INFO_TOTAL_AVAILABLE_MEMORY_NVX) * 1024)
              - this.totalDedicatedMemory; // Since its in Kb
    } else {
    ...
    for (int lvl = 0; lvl <= 1; lvl++) {
      ...
      for (int i = 0; i <= 10; i++) {
        ...
        nglClearNamedBufferSubData(
            buffer, GL_R32F, 0, 4096, GL_RED, GL_FLOAT, 0); // Zero the buffer
  ...
  public long getFreeDedicatedGpuMemory() {
    ...
    return glGetInteger64(GL_GPU_MEMORY_INFO_CURRENT_AVAILABLE_VIDMEM_NVX)
        * 1024; // Since its in Kb
  }
