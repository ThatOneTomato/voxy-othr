...
public abstract class AbstractRenderPipeline extends TrackedObject {
  ...
  private final FullscreenBlit depthStencilSetup =
      new FullscreenBlit("voxy:post/fullscreen2.vert", "voxy:post/setup_stencil_depth.frag");
  ...
  public final void setSectionRenderer(
      AbstractSectionRenderer<?, ?>
          sectionRenderer) { // Stupid java ordering not allowing something pre super
  ...
  protected abstract int setup(
      Gl46Viewport<?> viewport, int sourceFramebuffer, int srcWidth, int srcHeight);
  ...
  protected void initDepthStencil(
      int sourceFrameBuffer, int targetFb, int srcWidth, int srcHeight, int width, int height) {
    glClearNamedFramebufferfi(targetFb, GL_DEPTH_STENCIL, 0, 1.0f, 1);
    // using blit to copy depth from mismatched depth formats is not portable so instead a full
    // screen pass is performed for a depth copy
  ...
  protected static void transformBlitDepth(
      FullscreenBlit blitShader,
      int srcDepthTex,
      int dstFB,
      Gl46Viewport<?> viewport,
      Matrix4f targetTransform) {
    // at this point the dst frame buffer doesn't have a stencil attachment so we don't need to keep
    // the stencil test on for the blit
    // in the worst case the dstFB does have a stencil attachment causing this pass to become
    // 'corrupted'
    glDisable(GL_STENCIL_TEST);
    ...
    targetTransform.getToAddress(
        SCRATCH); // new Matrix4f(tooProjection).mul(vp.modelView).get(data);
