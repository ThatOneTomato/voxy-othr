package me.cortex.voxy.client.core.rendering.backend.gl46.section.mdic;
...
public class MDICSectionRenderer
  ...
  public static final Factory<MDICViewport, BasicSectionGeometryData> FACTORY =
      AbstractSectionRenderer.Factory.create(MDICSectionRenderer.class);
  ...
  private static final int TEMPORAL_OFFSET =
      TRANSLUCENT_OFFSET + TRANSLUCENT_DRAW_COUNT; // in draw calls
  ...
  private final Shader prepShader =
      Shader.make().add(ShaderType.COMPUTE, "voxy:lod/gl46/prep.comp").compile();
  ...
  private final Shader prefixSumShader =
      Shader.make()
          ...
          .add(
              ShaderType.COMPUTE,
              Capabilities.INSTANCE.subgroup
                  ? "voxy:lod/gl46/prefixsum/inital3.comp"
                  : "voxy:lod/gl46/prefixsum/simple.comp")
          .define("IO_BUFFER", 0)
          .compile();
  ...
  private final GlBuffer distanceCountBuffer =
      new GlBuffer(1024 * 4 + TRANSLUCENT_DRAW_COUNT * 4).zero(); // TODO move to viewport?
  ...
  public MDICSectionRenderer(
    ...
    var builder =
        Shader.make()
            ...
            // .defineIf("USE_NV_JANK", Capabilities.INSTANCE.isNvidia)//TODO: fix use capability to
            // try compile the jank thing to see if it can be and use that
    ...
    this.translucentTerrainShader =
        tryCompilePatchedOrNormal(builder.define("TRANSLUCENT"), translucentFrag, frag);
  ...
  private void uploadUniformBuffer(MDICViewport viewport) {
    ...
    mat.translate(
        -viewport.innerTranslation.x, -viewport.innerTranslation.y, -viewport.innerTranslation.z);
  ...
  private void renderTerrain(
      MDICViewport viewport, long indirectOffset, long drawCountOffset, int maxDrawCount) {
    ...
    glMemoryBarrier(
        GL_COMMAND_BARRIER_BIT | GL_SHADER_STORAGE_BARRIER_BIT); // Barrier everything is needed
    ...
    glMultiDrawElementsIndirectCountARB(
        GL_TRIANGLES, GL_UNSIGNED_SHORT, indirectOffset, drawCountOffset, maxDrawCount, 0);
    if (VoxyClient.getOcclusionDebugState() == 3) {
  ...
  public void buildDrawCalls(MDICViewport viewport) {
    if (this.geometryManager.getSectionCount() == 0) return;
    this.uploadUniformBuffer(viewport);
    // Can do a sneeky trick, since the sectionRenderList is a list to things to render, it invokes
    // the culler
    ...
    { // Generate the commands
      ...
      if (RenderStatistics.enabled) {
        ...
        glBindBufferBase(
            GL_SHADER_STORAGE_BUFFER, STATISTICS_BUFFER_BINDING, this.statisticsBuffer.id);
      }
      ...
      if (RenderStatistics.enabled) {
        DownloadStream.INSTANCE.download(
            ...
            down -> {
              ...
              for (int i = 0; i < LAYERS; i++) {
                RenderStatistics.quadCount[i] =
                    MemoryUtil.memGetInt(down.address + LAYERS * 4L + i * 4L);
              }
            });
      }
    }
    ...
    { // Do translucency sorting
      ...
      glBindBuffer(
          ...
          viewport
              .drawCountCallBuffer
              .id); // This isnt great but its a nice trick to bound it, even if its inefficent ;-;
      glMemoryBarrier(
          GL_COMMAND_BARRIER_BIT | GL_SHADER_STORAGE_BARRIER_BIT | GL_UNIFORM_BARRIER_BIT);
      glDispatchComputeIndirect(0);
      glMemoryBarrier(GL_COMMAND_BARRIER_BIT | GL_SHADER_STORAGE_BARRIER_BIT);
    }
  }
  ...
  public void addDebug(List<String> lines) {
    ...
    // (this.geometryManager.getGeometryUsed()/(1024*1024)));//section count/geometry size (MB)
  }
