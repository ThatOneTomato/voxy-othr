...
class NodeChildRequest {
  ...
  private final byte[] childChildExistence =
  ...
  public void setChildChildExistence(int childIdx, byte childExistence) {
    if ((this.mask & (1 << childIdx)) == 0) {
      throw new IllegalStateException(
          "Tried setting child child existence in request when child isnt in mask");
    }
  ...
  public boolean hasChildChildExistence(int childId) {
    if ((this.mask & (1 << childId)) == 0) {
      throw new IllegalStateException(
          "Tried getting child child existence set of child not in mask");
    }
  ...
  public byte getChildChildExistence(int childIdx) {
    if (!this.hasChildChildExistence(childIdx)) {
      throw new IllegalStateException(
          "Tried getting child child existence when child child existence for child was not set");
    }
  ...
  public int setChildMesh(int childIdx, int mesh) {
    ...
    // Note the mesh can be -ve meaning empty mesh, but we should still mark that node as having a
    // result
