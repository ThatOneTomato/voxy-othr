...
public class VoxyRenderSystem {
    ...
    public RenderFrame setupFrame(ChunkRenderMatrices matrices, double cameraX, double cameraY, double cameraZ) {
    ...
    public RenderFrame runFrameStage(
        ...
        return this.runFrameStage(stage, frame, matrices, cameraX, cameraY, cameraZ, irisActive, shaderPackActive, null);
    }
    ...
    private RenderFrameContext captureFrameContext(ChunkRenderMatrices matrices, double cameraX, double cameraY, double cameraZ) {
        int[] dims = new int[4];
     ...
     * sampled as {@code .r} for [0,1] depth), or 0 if the backend doesn't expose one. Used by
     * {@code MixinIrisSamplers} to bind {@code vxDepthTexOpaque} / {@code vxDepthTexTrans};
     * callers fall back to the Iris depth target when this is 0.
     */
