...
public class AutoBindingShader extends Shader {
  ...
  public AutoBindingShader ssbo(int index, GlBuffer buffer, long offset) {
    this.insertOrReplaceBinding(
        new BufferBinding(GL_SHADER_STORAGE_BUFFER, index, buffer, offset, -1));
    return this;
  }
    ...
    if (!this.bindings.isEmpty()) {
      for (var binding : this.bindings) {
        ...
        if (binding.offset == 0 && binding.size == -1) {
          ...
          glBindBufferRange(
              binding.target, binding.index, binding.buffer.id, binding.offset, binding.size);
        }
      }
    }
