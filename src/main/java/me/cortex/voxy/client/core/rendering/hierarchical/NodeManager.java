...
// TODO FIXME: CIRTICAL ISSUE: if a node is a top level section and is empty, when a child is tried
// to be made it explodes
...
// (issues related to this fix, lod updates from 0 children state to something children state,
// aswell as other way round)
...
public class NodeManager {
  ...
  // the child request,
  // however, since there are no children in it, it sticks around, since there isnt anything to
  // update it and invoke
  // the finishRequest on it
  // if the top level node ends up being updated with a child update, it should automatically solve
  // itself
  ...
  private final ExpandingObjectAllocationList<SingleNodeRequest> singleRequests =
      new ExpandingObjectAllocationList<>(SingleNodeRequest[]::new, NodeStore.REQUEST_ID_MSK);
  private final ExpandingObjectAllocationList<NodeChildRequest> childRequests =
      new ExpandingObjectAllocationList<>(NodeChildRequest[]::new, NodeStore.REQUEST_ID_MSK);
  ...
  private void clearAllocId(int id) {
    if (this.cleanerInterface != null) this.cleanerInterface.alloc(id);
  }

  private void clearMoveId(int from, int to) {
    if (this.cleanerInterface != null) this.cleanerInterface.move(from, to);
  }

  private void clearFreeId(int id) {
    if (this.cleanerInterface != null) this.cleanerInterface.free(id);
  }
  ...
  private static void assertPosValid(long pos) {
    ...
    if (WorldEngine.getLevel(p2) != 0
        || WorldEngine.getX(p2) != x
        || WorldEngine.getY(p2) != y
        || WorldEngine.getZ(p2) != z) {
      throw new IllegalStateException(
          "Position not valid at all levels: "
              + pos
              + "-"
              + WorldEngine.pprintPos(pos)
              + ":"
              + WorldEngine.pprintPos(p2));
    }
  }
  ...
  public void removeTopLevelNode(long pos) {
    if (!this.topLevelNodes.remove(pos)) {
      throw new IllegalStateException(
          "Position not in top level map: " + WorldEngine.pprintPos(pos));
    }
    int nodeId = this.activeSectionMap.get(pos);
    if (nodeId == -1) {
      throw new IllegalStateException(
          "Tried removing top level pos "
              + WorldEngine.pprintPos(pos)
    ...
    if ((nodeId & NODE_TYPE_MSK) != NODE_TYPE_REQUEST) {
      ...
      if (!this.topLevelNodeIds.remove(id)) {
        throw new IllegalStateException(
            "Node id was not in top level node ids: "
                + nodeId
                + " pos: "
                + WorldEngine.pprintPos(pos));
      }
      if (this.topLevelNodeIdRemovedCallback != null) this.topLevelNodeIdRemovedCallback.accept(id);
    }
  ...
  public void processGeometryResult(BuiltSection sectionResult) {
    ...
    int nodeId = this.activeSectionMap.get(pos);
    if (nodeId == -1) {
      // Logger.warn("Got geometry update for pos " + WorldEngine.pprintPos(pos) + " but it was not
      // in active map, discarding!");
      sectionResult.free();
      return;
    }
    ...
    if ((nodeId & NODE_TYPE_MSK) == NODE_TYPE_REQUEST) {
      ...
      if ((nodeId & REQUEST_TYPE_MSK) == REQUEST_TYPE_SINGLE) {
        ...
        // sectionResult has a cheeky childExistence field that we can use to set the request too,
        // this is just
        // because processChildChange is only ever invoked when child existence changes, so we still
        // need to
        ...
        request.setChildMesh(
            childId, this.uploadReplaceSection(request.getChildMesh(childId), sectionResult));
        ...
        || (nodeId & NODE_TYPE_MSK) == NODE_TYPE_LEAF) {
      nodeId &= NODE_ID_MSK;
  ...
  public void processChildChange(long pos, byte childExistence) {
    ...
    if ((nodeId & NODE_TYPE_MSK) == NODE_TYPE_REQUEST) {
      ...
      if (this.nodeData.isNodeRequestInFlight(nodeId & NODE_ID_MSK)) {
        //  Logger.error("UNFINISHED OPERATION TODO: FIXME: painful operation, needs to account for
        // both adding and removing, need to do the same with inner node, but also create requests,
        // or cleanup children");
        int requestId = this.nodeData.getNodeRequest(nodeId);
        var request =
            this.childRequests.get(
                requestId); // TODO: do not assume request is childRequest (it will probably always
        // be)
        if (request.getPosition() != pos)
          throw new IllegalStateException(
              "Request is not at pos, got "
        ...
        { // Update the request
          ...
          { // Remove children and free/release associated meshes
            ...
            for (int i = 0; i < 8; i++) {
              ...
              if (this.activeSectionMap.remove(cPos)
                  == -1) { // TODO: verify the removed section is a request type of child and the
                // request id matches this
                throw new IllegalStateException(
          ...
          { // Add new children to the request
            ...
            for (int i = 0; i < 8; i++) {
              ...
              long cPos = makeChildPos(pos, i);
              if (this.activeSectionMap.put(
                      cPos, requestId | NODE_TYPE_REQUEST | REQUEST_TYPE_CHILD)
                  != -1) {
                throw new IllegalStateException(
  ...
  private void updateChildSectionsInner(long pos, int nodeId, byte childExistence) {
    ...
    // This works in 2 parts, adding and removing, adding is (surprisingly) much easier than
    // removing
    ...
    if (add != 0) { // We have nodes to add
      if (!this.nodeData.isNodeRequestInFlight(
          nodeId)) { // If there is not an existing request, create it
      ...
      for (int i = 0; i < 8; i++) {
        ...
        // Add child to active tracker and put in updateRouter
        long cPos = makeChildPos(pos, i);
        if (this.activeSectionMap.put(cPos, requestId | NODE_TYPE_REQUEST | REQUEST_TYPE_CHILD)
            != -1) {
          throw new IllegalStateException(
              "Child pos was already in active section tracker but was part of a request");
        }
    ...
    // this needs to be before the removal since that may invoke requestFinish, which expects
    // updated node masks
    // TODO: verify this
    ...
    if (rem != 0) {
      ...
      if (this.nodeData.isNodeRequestInFlight(nodeId)) {
        ...
        if (reqRem != 0) {
          ...
          for (int i = 0; i < 8; i++) {
            ...
            if (cnid == -1
                || (cnid & NODE_TYPE_MSK)
                    != NODE_TYPE_REQUEST) { // TODO: verify the removed section is a request type of
              // child and the request id matches this
              throw new IllegalStateException(
                  "Child pos was in a request but not in active section map");
            }
            if (!this.watcher.unwatch(cPos, WorldEngine.DEFAULT_UPDATE_FLAGS)) {
      ...
      if (rem != 0) {
        ...
        if (remaining == 0) {
          // This state should only ever occur when a node is inflight, or... if an inner node has
          // existance mask of 0... sigh
          ...
          this.nodeData.setAllChildrenAreLeaf(
              nodeId, false); // Children dont exist, therefor set them to false
      ...
      if (this.nodeData.isNodeRequestInFlight(
          nodeId)) { // Also only need to do this after/if there are removals to be done
        int requestId = this.nodeData.getNodeRequest(nodeId);
        var request =
            this.childRequests.get(
                requestId); // TODO: do not assume request is childRequest (it will probably always
    ...
    if (childExistence == 0) {
      ...
      if (this.nodeData.isNodeRequestInFlight(
          nodeId)) // Leaf nodes cannot have requests associated to them
      throw new IllegalStateException();
      ...
      if (this.nodeData.getNodeGeometry(nodeId) == NULL_GEOMETRY_ID) {
        ...
        // TODO: figure out a better way to mark this for tracing verificaction and like less
        // confusion
        // (instead of like EMPTY_GEOMETRY_ID do like INFLIGHT_GEOMETRY_ID)
        this.nodeData.setNodeGeometry(nodeId, EMPTY_GEOMETRY_ID);
      }

      if (this.nodeData.getChildPtr(nodeId)
          != SENTINEL_EMPTY_CHILD_PTR) { // This should only ever be the sentinal ptr
        throw new IllegalStateException();
      }
  ...
  private void _removeRequest(int reqId, NodeChildRequest req, long pos) {
    ...
    for (int i = 0; i < 8; i++) {
      ...
      if ((cId & NODE_TYPE_MSK) != NODE_TYPE_REQUEST
          || (cId & REQUEST_TYPE_MSK) != REQUEST_TYPE_CHILD
  ...
  private void _recurseRemoveNode(long pos, boolean onlyRemoveChildren) {
    ...
    if (type == NODE_TYPE_INNER || type == NODE_TYPE_LEAF) {
      ...
      // Need to recurse into childExistence that exist, this is xor between a request mask if there
      // is and the
      // childRequest
      ...
      if ((nodeId & REQUEST_TYPE_MSK) == REQUEST_TYPE_SINGLE) {
        ...
        if (req.getPosition() != pos) throw new IllegalStateException();
  ...
  private void finishRequest(SingleNodeRequest request) {
    ...
    this.activeSectionMap.put(
        request.getPosition(),
        id | NODE_TYPE_LEAF); // Assume that the result of any single request type is a leaf node
    ...
    if (this.topLevelNodeIdAddedCallback != null) this.topLevelNodeIdAddedCallback.accept(id);
  }
    ...
    if (parentNodeType == NODE_TYPE_LEAF) {
      ...
      for (int childIdx = 0; childIdx < 8; childIdx++) {
        ...
        if (childExistence == 0) {
          ...
          Logger.warn(
              "Request result with child existence of 0, for child pos "
        ...
        if ((pid & NODE_TYPE_MSK) != NODE_TYPE_REQUEST) {
          throw new IllegalStateException(
              "Put node in map from request but type was not request: "
                  + pid
                  + " "
                  + WorldEngine.pprintPos(childPos));
        }
      ...
      // For this, only need to add the nodes to the existing child set thing (shuffle around
      // whatever) dont ever have to remove nodes
      ...
      if (oldChildPtr != SENTINEL_EMPTY_CHILD_PTR) {
        // Ok so technically, it _is ok_ to just add to the end of the childPtr, however, imo that
        // is stupid
        // and it should follow the logical allocation with respect to the 8 child indices
        // this means, need to extract the child indices already in the ptr (or technically could
        // use the child existance? but having both and doing verification would be good)
  ...
  public void processRequest(long pos) {
    ...
    if (nodeId == -1) {
      ...
      // Logger.warn("Got request for pos " + WorldEngine.pprintPos(pos) + " but it was not in
      // active map, ignoring!");
      return;
    }
    ...
    if (nodeType == NODE_TYPE_REQUEST) {
      Logger.error(
          "Tried processing request for pos: "
              + WorldEngine.pprintPos(pos)
    ...
    if (WorldEngine.getLevel(pos) == 0) {
      Logger.error(
          "Requests cannot exist for bottom level nodes. at: "
              + WorldEngine.pprintPos(pos)
              + ". Ignoring request");
      return;
    }
    ...
    // NOTE: inner nodes /w request should check they have geometry independenently of being
    // inflight

    // TODO: FIXTHIS:
    // https://discord.com/channels/973046939375505408/973046939375505411/1328785093812031489
    // this causes things to go bad, when racing the gpu, i.e. this becomes an inner node that has
    // geometry and there is now a request for it
    ...
    if (nodeType == NODE_TYPE_LEAF) {
      ...
      if (this.nodeData.getNodeGeometry(nodeId) == NULL_GEOMETRY_ID) {
        ...
        Logger.warn(
            "Got request for leaf that doesnt have geometry, this should not be possible at pos "
                + WorldEngine.pprintPos(pos));
      ...
      if (this.nodeData.isNodeRequestInFlight(nodeId)) {
        Logger.warn(
            "Tried processing a node that already has a request in flight: "
                + nodeId
                + " pos: "
                + WorldEngine.pprintPos(pos)
                + " ignoring");
  ...
  private void makeLeafChildRequest(int nodeId) {
    ...
    if (childExistence == 0) {
      if (!this.topLevelNodes.contains(
          pos)) { // Top level nodes are special, as they can have a request with child existence of
        // 0 for performance reasons
        Logger.warn(
            "Not creating a leaf request with existence mask of 0 at pos",
            WorldEngine.pprintPos(pos));
        this.nodeData.unmarkRequestInFlight(nodeId);
        this.invalidateNode(nodeId);
        return;
    ...
    // Only request against the childExistence mask, since the guarantee is that if childExistence
    // bit is not set then that child is guaranteed to be empty
    for (int i = 0; i < 8; i++) {
  ...
  private void processInnerRequest(long pos, int nodeId) {
    ...
    if (VERIFY_NODE_MANAGER_OPERATIONS) {
      ...
      if (inflight && !isWatchingUpdate) {
        throw new IllegalStateException(); // If we have geometry request inflight we must be
        // watching
      }
      if (geo != NULL_GEOMETRY_ID && inflight) {
        // Having a EMPTY_GEOMETRY_ID and inflight is valid unfortunatly due to conditions when
        // making an
        // inner node into a leaf node when child existance is set to zero and it has no geometry
        if (geo != EMPTY_GEOMETRY_ID) throw new IllegalStateException();
      }
    }

    if (!this.nodeData.isNodeGeometryInFlight(nodeId)) {
      if (!this.watcher.watch(pos, WorldEngine.UPDATE_TYPE_BLOCK_BIT)) {
        // Logger.info("Node: " + nodeId + " at pos: " + WorldEngine.pprintPos(pos) + " got update
        // request, but geometry was already being watched");
        this.invalidateNode(
            nodeId); // Who knows why but just invalidate the data just to keep in sync
      } else {
  ...
  // TODO: Think plan for this is to add new flag to NodeStore to indicate if geometry mesh request
  // is inflight
  // this used for state verification and not emitting/assuming things
  // e.g. current issue is if an inner node wants/needs to convert into a leaf node, but the inner
  // node has no geometry
  // how to deal with that?? e.g. inner node geometry gets cleared but then the childExistance gets
  // set to 0
  ...
  public void removeNodeGeometry(long pos) {
    int nodeId = this.activeSectionMap.get(pos);
    if (nodeId == -1) {
      // Logger.warn("Got geometry removal for pos " + WorldEngine.pprintPos(pos) + " but it was not
      // in active map, ignoring!");
      return;
    }
    int nodeType = nodeId & NODE_TYPE_MSK;
    nodeId &= NODE_ID_MSK;
    if (nodeType == NODE_TYPE_REQUEST) {
      ...
      // Logger.warn("Tried removing geometry for pos: " + WorldEngine.pprintPos(pos) + " but its
      // type was a request, ignoring!");
      return;
    }
    // this.clearId(nodeId);
  ...
  private void processLeafGeometryRemoval(long cPos) {
    ...
    if (false) { // Check all children are leaf nodes
      ...
      if (cPtr != SENTINEL_EMPTY_CHILD_PTR) {
        ...
        for (int i = 0; i < cCnt; i++) {
          if (!this.nodeData.nodeExists(i + cPtr)) throw new IllegalStateException();
          long cp = this.nodeData.nodePosition(i + cPtr);
          long cn = this.activeSectionMap.get(cp);
          if (cn == -1) throw new IllegalStateException();
    ...
    if (pGeo == NULL_GEOMETRY_ID) {
      ...
      this.recurseRemoveChildNodes(
          pPos); // TODO: make this download/fetch the data instead of just deleting it
      // this.clearId(pId);

      // Make node a leaf
      int old = this.activeSectionMap.put(pPos, NODE_TYPE_LEAF | pId);
      if (old == -1) throw new IllegalStateException();
  ...
  private void clearGeometryInternal(long pos, int nodeId) {
    ...
    if (meshId != NULL_GEOMETRY_ID && meshId != EMPTY_GEOMETRY_ID) {
      ...
      if (this.watcher.unwatch(pos, WorldEngine.UPDATE_TYPE_BLOCK_BIT)) {
        throw new IllegalStateException(
            "Unwatching position for geometry removal at: "
                + WorldEngine.pprintPos(pos)
                + " resulted in full removal");
      }
      ...
      this.nodeData.unmarkNodeGeometryInFlight(
          nodeId); // Remove geometry inflight as well, its removed
    } else {
  ...
  public void writeNode(int node, long address) {
  ...
  private static long makeParentPos(long pos) {
    ...
    if (lvl == MAX_LOD_LAYER) {
      throw new IllegalArgumentException(
          "Cannot create a parent higher than LoD " + (MAX_LOD_LAYER));
    }
    return WorldEngine.getWorldSectionId(
        lvl + 1,
  ...
  // however, if the node doesnt have geometry attached that would put it in an invalid state so
  // need to figure out
  // a solution for this
  ...
  private int verifyRequest(
    ...
    if (this.nodeData.isNodeRequestInFlight(node)) {
      ...
      int reqMsk = Byte.toUnsignedInt(request.getMsk());
      if ((cActiveExistence & reqMsk)
          != 0) // Cannot have an active child and request for the same position
      throw new IllegalStateException();
      for (int i = 0; i < 8; i++) {
  ...
  private void verifyNode(long pos, LongOpenHashSet seenPositions, IntOpenHashSet seenNodes) {
    ...
    if (!seenPositions.add(pos)) throw new IllegalStateException();
    ...
    if (type == NODE_TYPE_REQUEST) {
      ...
      if ((this.nodeData.getNodeRequest(node) != NULL_REQUEST_ID)
...

      {
        ...
        if ((hasGeometry || awaitingGeo) != watchingGeo) throw new IllegalStateException();
        // Cannot be awaiting geometry and have it
        if (hasGeometry && awaitingGeo) {
          // We assume if the geometry is EMPTY, that what happened was an inner node just got
          // convertex into a leaf node and is now awaiting its geometry
          // if (type != NODE_TYPE_LEAF || this.nodeData.getNodeGeometry(node) != EMPTY_GEOMETRY_ID)
          //    throw new IllegalStateException();
          // HOWEVER, what can happen is that before we recieve the geometry for the node, thus
          // clearing the geometryInFlight
      ...
      //    throw new IllegalStateException();
      // }
      if (!seenNodes.add(node)) throw new IllegalStateException();
      if (type == NODE_TYPE_INNER) {
        ...
        if (this.nodeData.getNodeGeometry(node)
            == NULL_GEOMETRY_ID) { // Leafs cannot have null geometry
          throw new IllegalStateException();
        }
        ...
        if (WorldEngine.getLevel(pos) == 0) {
          ...
          if (this.nodeData.isNodeRequestInFlight(
              node)) { // Child nodes cannot have inflight requests
            throw new IllegalStateException();
          }
        } else {
