...
public class BasicSectionGeometryData implements GeometryData {
  ...
  public BasicSectionGeometryData(int maxSectionCount, long geometryCapacity) {
    ...
    String msg =
        "Creating and zeroing " + (geometryCapacity / (1024 * 1024)) + "MB geometry buffer";
    if (Capabilities.INSTANCE.canQueryGpuMemory) {
      msg +=
          " driver states "
              + (Capabilities.INSTANCE.getFreeDedicatedGpuMemory() / (1024 * 1024))
              + "MB of free memory";
    }
    Logger.info(msg);
    Logger.info(
    ...
    if (!(Capabilities.INSTANCE.isNvidia
        && ThreadUtils.isWindows
        && Capabilities.INSTANCE
            .sparseBuffer)) { // This hack makes it so it doesnt crash on renderdoc
      buffer = new GlBuffer(geometryCapacity, false); // Only do this if we are not on nvidia
      // TODO: FIXME: TEST, see if the issue is that we are trying to zero the entire buffer, try
      // only zeroing increments
    ...
    if (error != GL_NO_ERROR || buffer == null) {
      if ((buffer == null || error == GL_OUT_OF_MEMORY) && Capabilities.INSTANCE.sparseBuffer) {
        if (buffer != null) {
          Logger.error(
              "Failed to allocate geometry buffer, attempting workaround with sparse buffers");
          buffer.free();
        }
        ...
        throw new IllegalStateException(
            "Unable to allocate geometry buffer, got gl error " + error);
      }
    }
  ...
  public void ensureAccessable(int maxElementAccess) {
    ...
    if (this.geometryBuffer.isSparse()) {
      if (this.sparseCommitment
          < size) { // if we try to access memory outside the allocation range, allocate it
        ...
        glBufferPageCommitmentARB(
            GL_ARRAY_BUFFER, this.sparseCommitment, size - this.sparseCommitment, true);
        glBindBuffer(GL_ARRAY_BUFFER, 0);
        // Logger.info("Resizing sparse: " + this.sparseCommitment + ", " +
        // (size-this.sparseCommitment));
  ...
  public void free() {
    ...
    if (!this.isExternalGeometryBuffer) {
      ...
      if (Capabilities.INSTANCE.canQueryGpuMemory) {
        long releaseSize =
            (long)
                (this.geometryBuffer.size()
                    * 0.75); // if gpu memory usage drops by 75% of the expected value assume we
        // freed it
        if (this.geometryBuffer
            .isSparse()) { // If we are using sparse buffers, use the commited size instead
        ...
        if (Capabilities.INSTANCE.getFreeDedicatedGpuMemory() - gpuMemory <= releaseSize) {
          ...
          while (System.currentTimeMillis() - start
              < TIMEOUT) { // Wait up to 2.5 seconds for memory to release
            glFinish();
