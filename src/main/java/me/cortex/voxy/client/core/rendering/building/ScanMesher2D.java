...
public abstract class ScanMesher2D {
  ...
  // Two different ways to do it, scanline then only merge on change, or try to merge with previous
  // row at every step
  // or even can also attempt to merge previous but if the lengths are different split the current
  // one and merge to previous
  ...
  private void putNext0(long data) {
    int idx =
        (this.currentIndex++)
            & 31; // Mask to current row, but keep total so can compute actual indexing

    // If we are on the zero index, ignore it as we are going from empty state to maybe something
    // state
    // setup data
    if (idx == 0) {
    ...
    if (isSet) {
      this.emitQuad(
          idx & 31,
          ((this.currentIndex - 1) >> 5) - (causedByDepthMax ? 0 : 1),
          this.rowLength[idx],
          this.rowDepth[idx],
          this.rowData[idx]);
  ...
  private void emitRanged(int msk) {
    { // Emit quads that cover the previous indices
      ...
      this.rowBitset &= ~msk;
      while (rowSet
          != 0) { // Need to emit quads that would have skipped, note that this does not include the
        // current index
        int index = Integer.numberOfTrailingZeros(rowSet);
        rowSet &= ~Integer.lowestOneBit(rowSet);

        // Emit the quad, dont need to clear the data since it not existing in the bitmask is
        // implicit no data
        this.emitQuad(
            index,
    ...
    var mesher =
        new ScanMesher2D() {
          ...
          protected void emitQuad(int x, int z, int length, int width, long data) {
            ...
            if (length * width != 1) {
              if (data != qc[0]) throw new IllegalStateException();
            }
            if (x != (((qc[0]) & 0x1f))) throw new IllegalStateException();
            if (z != ((qc[0]) >> 5)) throw new IllegalStateException();
          }
        };
