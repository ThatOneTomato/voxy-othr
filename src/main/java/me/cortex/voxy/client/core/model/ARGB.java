...
public class ARGB {
  ...
  private static float computeLinearToSrgb(float f) {
    return f >= 0.0031308F
        ? (float) (1.055 * Math.pow((double) f, 0.4166666666666667) - 0.055)
        : 12.92F * f;
  }
  ...
  public static int meanLinear(int i, int j, int k, int l) {
    return color(
        (alpha(i) + alpha(j) + alpha(k) + alpha(l)) / 4,
        linearChannelMean(red(i), red(j), red(k), red(l)),
        linearChannelMean(green(i), green(j), green(k), green(l)),
        linearChannelMean(blue(i), blue(j), blue(k), blue(l)));
  }
  ...
  public static int color(Vec3 vec3) {
    return color(
        as8BitChannel((float) vec3.x()),
        as8BitChannel((float) vec3.y()),
        as8BitChannel((float) vec3.z()));
  }
  ...
  public static int addRgb(int i, int j) {
    return color(
        alpha(i),
        Math.min(red(i) + red(j), 255),
        Math.min(green(i) + green(j), 255),
        Math.min(blue(i) + blue(j), 255));
  }

  public static int subtractRgb(int i, int j) {
    return color(
        alpha(i),
        Math.max(red(i) - red(j), 0),
        Math.max(green(i) - green(j), 0),
        Math.max(blue(i) - blue(j), 0));
  }
  ...
  public static int scaleRGB(int i, float f, float g, float h) {
    return color(
        alpha(i),
        ARGB.clamp((long) ((int) ((float) red(i) * f)), 0, 255),
        ARGB.clamp((long) ((int) ((float) green(i) * g)), 0, 255),
        ARGB.clamp((long) ((int) ((float) blue(i) * h)), 0, 255));
  }

  public static int scaleRGB(int i, int j) {
    return color(
        alpha(i),
        ARGB.clamp((long) red(i) * (long) j / 255L, 0, 255),
        ARGB.clamp((long) green(i) * (long) j / 255L, 0, 255),
        ARGB.clamp((long) blue(i) * (long) j / 255L, 0, 255));
  }
  ...
  public static int alphaBlend(int i, int j) {
    ...
    if (l == 255) {
      ...
      return color(
          m,
          alphaBlendChannel(m, l, red(i), red(j)),
          alphaBlendChannel(m, l, green(i), green(j)),
          alphaBlendChannel(m, l, blue(i), blue(j)));
    }
  }
  ...
  public static int linearLerp(float f, int i, int j) {
    return color(
        Mth.lerpInt(f, alpha(i), alpha(j)),
        LINEAR_TO_SRGB[Mth.lerpInt(f, SRGB_TO_LINEAR[red(i)], SRGB_TO_LINEAR[red(j)])] & 255,
        LINEAR_TO_SRGB[Mth.lerpInt(f, SRGB_TO_LINEAR[green(i)], SRGB_TO_LINEAR[green(j)])] & 255,
        LINEAR_TO_SRGB[Mth.lerpInt(f, SRGB_TO_LINEAR[blue(i)], SRGB_TO_LINEAR[blue(j)])] & 255);
  }
