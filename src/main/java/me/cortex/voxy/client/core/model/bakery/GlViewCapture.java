...
public class GlViewCapture {
  ...
  public GlViewCapture(int width, int height) {
    ...
    this.height = height;
    this.metaTex =
        new GlTexture().store(GL_R32UI, 1, width * 3, height * 2).name("ModelBakeryMetadata");
    this.colourTex =
        new GlTexture().store(GL_RGBA8, 1, width * 3, height * 2).name("ModelBakeryColour");
    this.depthTex =
        new GlTexture()
            .store(GL_DEPTH24_STENCIL8, 1, width * 3, height * 2)
            .name("ModelBakeryDepth");
    ...
    this.framebuffer =
        new GlFramebuffer()
            ...
            .bind(GL_COLOR_ATTACHMENT1, this.metaTex)
  ...
  public void emitToStream(int buffer, int offset) {
    ...
    glBindBufferRange(
        ...
        buffer,
        offset,
        (this.width * 3L) * (this.height * 2L) * 4L * 2); // its 2*4 because colour + depth stencil
    glMemoryBarrier(
        GL_FRAMEBUFFER_BARRIER_BIT
            | GL_TEXTURE_UPDATE_BARRIER_BIT
            | GL_PIXEL_BUFFER_BARRIER_BIT
            | GL_SHADER_IMAGE_ACCESS_BARRIER_BIT); // Am not sure if barriers are right
    ...
    glBindBufferRange(
        ...
        0,
        0,
        4); // WHY DOES THIS FIX FUCKING BINDING ISSUES HERE WHEN DOING THIS IN THE RENDER SYSTEM
    // DOESNT
  }
