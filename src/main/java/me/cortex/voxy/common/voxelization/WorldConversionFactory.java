...
public class WorldConversionFactory {
  private static final boolean LITHIUM_INSTALLED = PlatformAccess.get().isModLoaded("lithium");

  private static final class Cache {
    private final int[] biomeCache = new int[4 * 4 * 4];
    private final WeakHashMap<Mapper, Reference2IntOpenHashMap<BlockState>> localMapping =
        new WeakHashMap<>();
  ...
  private static boolean setupLithiumLocalPallet(
      Palette<BlockState> vp,
  ...
  public static VoxelizedSection convert(
      ...
      for (int y = 0; y < 4; y++) {
        for (int z = 0; z < 4; z++) {
          for (int x = 0; x < 4; x++) {
            ...
            shouldZoom &=
                inital == bid; // Evil hacky trick, we only need to zoom if on a biome boarder
          }
        }
      }
    ...
    if (blockContainer.data.storage() instanceof SimpleBitStorage bStor) {
      ...
      for (int i = 0; i <= 0xFFF; i++) {
        ...
        data[i] =
            Mapper.composeMappingId(light, bId, biomes[Integer.compress(i, 0b1100_1100_1100)]);
      }
    } else {
      ...
      if (bId == 0) { // Its air
        for (int i = 0; i <= 0xFFF; i++) {
          data[i] =
              Mapper.airWithLight(lightSupplier.supply(i & 0xF, (i >> 8) & 0xF, (i >> 4) & 0xF));
        }
      } else {
        nonZeroCnt = 4096;
        for (int i = 0; i <= 0xFFF; i++) {
