...
public class RocksDBStorageBackend extends StorageBackend {
  ...
  public RocksDBStorageBackend(String path) {
    ...
    final ColumnFamilyOptions cfWorldSecOpts =
        new ColumnFamilyOptions()
  ...
  public void iteratePositions(int level, LongConsumer consumer) {
    try (var stack = MemoryStack.stackPush()) {
      try (var iter = this.db.newIterator(this.worldSections, this.sectionReadOps)) {
        ...
        // TODO: this can be optimized if needed by useing a prefix-seek
        // https://github.com/facebook/rocksdb/wiki/Prefix-Seek

        if (level != -1) { // -1 means iterate all
          var seekBuff = stack.calloc(8);
          MemoryUtil.memPutLong(
              MemoryUtil.memAddress(seekBuff),
  ...
  private static int bytesToInt(byte[] i) {
    return (Byte.toUnsignedInt(i[0]) << 24)
        | (Byte.toUnsignedInt(i[1]) << 16)
        | (Byte.toUnsignedInt(i[2]) << 8)
  ...
  public static class Config extends StorageConfig {
    ...
    public StorageBackend build(ConfigBuildCtx ctx) {
      return new RocksDBStorageBackend(
          ctx.ensurePathExists(ctx.substituteString(ctx.resolvePath())));
    }
  ...
  private static long swizzlePos(long key) {
    ...
    return (key & (0xFL << 60))
        ...
        | Long.expand(
            (key >>> 52) & 0xFF, 0b00000000000000000000000000000000_100100100100100100100100L)
        | Long.expand(
            (key >>> 28) & ((1L << 24) - 1),
