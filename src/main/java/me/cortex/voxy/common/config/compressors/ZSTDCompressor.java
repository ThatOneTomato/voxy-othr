...
public class ZSTDCompressor implements StorageCompressor {
  ...
  private static Ref createCleanableDecompressionContext() {
    ...
    nZSTD_DCtx_setParameter(
        ctx, ZSTD_d_experimentalParam3, 1); // experimental ZSTD_d_forceIgnoreChecksum
    var ref = new Ref(ctx);
  ...
  private static final ResizingThreadLocalMemoryBuffer SCRATCH =
      new ResizingThreadLocalMemoryBuffer(
  ...
  public MemoryBuffer compress(MemoryBuffer saveData) {
    var compressedData =
        SCRATCH.get(ZSTD_COMPRESSBOUND(saveData.size)).createUntrackedUnfreeableReference();
    long compressedSize =
        nZSTD_compressCCtx(
            COMPRESSION_CTX.get().ptr,
