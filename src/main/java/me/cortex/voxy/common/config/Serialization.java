...
public class Serialization {
  ...
  private static final class GsonConfigSerialization<T> implements TypeAdapterFactory {
    ...
    public GsonConfigSerialization<T> register(String typeName, Class<? extends T> cls) {
      ...
      if (this.type2name.put(cls, typeName) != null) {
        throw new IllegalStateException(
            "Class already registered with type name: " + typeName + ", " + cls);
      }
      return this;
    }
    ...
    private JsonElement serialize(Gson gson, T value) {
      ...
      var vjson =
          gson.getDelegateAdapter(this, TypeToken.get((Class<T>) value.getClass()))
              .toJsonTree(value);
      ...
      try {
        ...
        while ((clz = clz.getSuperclass()) != null) {
          if (CONFIG_TYPES.contains(clz)) {
            ...
            serializers
                .computeIfAbsent(clz, GsonConfigSerialization::new)
  ...
  private static List<String> collectAllClasses(String pack) {
    try {
      InputStream stream =
          Serialization.class.getClassLoader().getResourceAsStream(pack.replaceAll("[.]", "/"));
      if (stream == null) {
        // Module classloaders (neoforge) don't expose package listings as resources;
        // the mod-root path scan handles that case instead.
        return List.of();
      }
  ...
  private static List<String> collectAllClasses(Path base, String pack) {
    ...
    try {
      return Files.list(base.resolve(pack.replaceAll("[.]", "/")))
          .flatMap(
              inner -> {
                if (inner.getFileName().toString().endsWith(".class")) {
                  return Stream.of(
                      pack + "." + inner.getFileName().toString().replace(".class", ""));
