...
public class LMDBStorageBackend extends StorageBackend {
  ...
  private <T> T synchronizedTransaction(Supplier<T> transaction) {
    try {
      ...
      // Check if its locked, if it is locked then need to release the access, wait till resize is
      // finished then
  ...
  public MemoryBuffer getSectionData(long key, MemoryBuffer scratch) {
    return this.synchronizedTransaction(
        () ->
            this.sectionDatabase.transaction(
                MDB_RDONLY,
                transaction -> {
        ...
        () ->
            this.sectionDatabase.transaction(
                transaction -> {
                  ...
                  transaction.put(
                      keyBuff, MemoryUtil.memByteBuffer(data.address, (int) data.size), 0);
                  return null;
                }));
  }

  public void deleteSectionData(long key) {
    this.synchronizedTransaction(
        () ->
            this.sectionDatabase.transaction(
                transaction -> {
                  var keyBuff = transaction.stack.malloc(8);
  ...
  public synchronized void putIdMapping(int id, ByteBuffer data) {
    this.resizingTransaction(
        () ->
            this.idMappingDatabase.transaction(
                transaction -> {
