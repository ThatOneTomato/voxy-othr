...
public class WorldUpdater {
  // Executes an update to the world and automatically updates all the parent mip layers up to level
  // 4 (e.g. where 1 chunk section is 1 block big)

  // NOTE: THIS RUNS ON THE THREAD IT WAS EXECUTED ON, when this method exits, the calling method
  // may assume that VoxelizedSection is no longer needed
  public static void insertUpdate(
      WorldEngine into,
      VoxelizedSection
          section) { // TODO: add a bitset of levels to update and if it should force update
    ...
    if (VoxyFlags.IS_MINE_IN_ABYSS) {
      ...
      section.setPosition(
          section.x - (sector << 10),
          section.y + 16 + (256 - 32 - sector * 30),
          section.z); // Note sector size mult is 30 because the top chunk is replicated (and so is
      // bottom chunk)
    }
    ...
    for (int lvl = 0; lvl <= MAX_LOD_LAYER; lvl++) {
      ...
      if (didStateChange || (emptinessStateChange != 0)) {
        // TODO: somehow foward the neighbors that are facing the updated area, this allows
        // forwarding to the dirty consumer
        // which can decide wether to dispatch mesh rebuilds to the surounding sections
        // Bitmask of neighboring sections
        // Note, this may be zero (this is more likely to occure at higher lod levels) if it doesnt
        // face any neighbors
        int neighbors = 0;
        if (didStateChange) {
        ...
        into.markDirty(
            worldSection,
            (didStateChange ? UPDATE_TYPE_BLOCK_BIT : 0)
  ...
  private static long insertSectionLvlIntoWorld(
      VoxelizedSection section, WorldSection worldSection) {
    ...
    status |=
        Integer.toUnsignedLong(airCount)
            << 1; // VERY VERY VERY IMPORTANT NOTE: IS 13 BITS BIG NOT 12 BITS (since it can be 4096
    // which is 6 bits large)
    return status;
  }
}
