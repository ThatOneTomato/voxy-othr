...
public class VoxelIngestService {
  ...
  private void processJob() {
    ...
    if (section.hasOnlyAir()
        && task.blockLight == null
        && task.skyLight == null) { // If the chunk section has lighting data, propagate it
  ...
  public boolean enqueueIngest(WorldEngine engine, LevelChunk chunk) {
    ...
    for (var section : chunk.getSections()) {
      ...
      if (lightingProvider.getDebugSectionType(LightLayer.SKY, pos)
              != LayerLightSectionStorage.SectionType.LIGHT_AND_DATA
          && lightingProvider.getDebugSectionType(LightLayer.BLOCK, pos)
              != LayerLightSectionStorage.SectionType.LIGHT_AND_DATA) continue;
      gotLighting = true;
    }
    ...
    if (allEmpty && !gotLighting) {
      ...
      for (var section : chunk.getSections()) {
        ...
        this.ingestQueue.add(
            new IngestSection(chunk.getPos().x, i, chunk.getPos().z, engine, section, null, null));
        try {
    ...
    for (var section : chunk.getSections()) {
      ...
      this.ingestQueue.add(
          new IngestSection(
              ...
              section,
              bl,
              sl)); // TODO: fixme, this is technically not safe todo on the chunk load ingest, we
      // need to copy the section data so it cant be modified while being read
      try {
        this.service.execute();
      } catch (Exception e) {
        Logger.error("Executing had an error: assume shutting down, aborting", e);
  ...
  public static boolean rawIngest(
      WorldIdentifier id,
      LevelChunkSection section,
      int x,
      int y,
