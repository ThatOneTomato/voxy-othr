...
public class ActiveSectionTracker {
      ...
      try {
        PRE_ACQUIRE_COUNT =
            MethodHandles.lookup()
                .findVarHandle(VolatileHolder.class, "preAcquireCount", int.class);
        POST_ACQUIRE_COUNT =
            MethodHandles.lookup()
                .findVarHandle(VolatileHolder.class, "postAcquireCount", int.class);
  ...
  private final Long2ObjectLinkedOpenHashMap<WorldSection>
      lruSecondaryCache; // TODO: THIS NEEDS TO BECOME A GLOBAL STATIC CACHE
  ...
  public ActiveSectionTracker(
      int numSlicesBits, SectionLoader loader, int cacheSize, WorldEngine engine) {
    this.engine = engine;
  ...
  public WorldSection acquire(long key, boolean nullOnEmpty) {
    // TODO: add optional verification check to ensure this (or other critical systems) arnt being
    // called on the render or server thread
...

    {
      ...
      if (holder != null) { // Return already loaded entry
        ...
        var eHolder =
            cache.putIfAbsent(
                key,
                holder); // We put if absent because on failure to convert to write, it leaves race
        // condition
    ...
    if (isLoader) {
      ...
      if (section == null
          && (!this.lruSecondaryCache.isEmpty())
          && this.lruSize + 100
              < this.lruSecondaryCache.size()
                  + this.getLoadedCacheCount()) { // Add a self clamping lru case for when there are
        // alot of loaded sections
    ...
    if (isLoader) {
      ...
      if (section == null) { // Secondary cache miss
        ...
        // TODO: REWRITE THE section tracker _again_ to not be so shit and jank, and so that
        // Arrays.fill is not 10% of the execution time
        if (status == 1) {
      ...
      if (nullOnEmpty
          && status == 1) { // If its air return null as stated, release the section aswell
        section.release();
        return null;
      }
      return section;
    } else {
      // TODO: mark the time the loading started in nanos, then here if it has been a while, spin
      // lock, else jump back to the executing service and do work
      VarHandle.fullFence();
  ...
  void tryUnload(WorldSection section, int hints) {
    ...
    boolean shouldRetryExit = false;
    {
      ...
      if (this.engine != null && section.shouldSave()) { // Last call for saving
        if (section.tryAcquire()) {
          if (!this.engine.saveSection(
              section, true, true)) { // not allowed to block as we are in a lock
            ...
            VarHandle.fullFence();
            shouldRetryExit |=
                section.isDirty; // or if the section is now dirty, note this must go AFTER the ref
            // check, since you can only mark live sections as dirty
          ...
          // if another thread marks this chunk as dirty (it would have acquired it after the inital
          // `section.getRefCount() != 0`
          ...
          throw new IllegalStateException(
              "Section was dirty but is also unloaded, this is very bad");
        }
      }
        ...
        if (obj != section) {
          throw new IllegalStateException(
              "Removed section not the same as the referenced section in the cache: cached: "
                  ...
                  + section
                  + " A: "
                  + WorldSection.ATOMIC_STATE_HANDLE.get(obj)
                  + " B: "
                  + WorldSection.ATOMIC_STATE_HANDLE.get(section));
        }
        sec = section;
      }
    }
