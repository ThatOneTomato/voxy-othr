...
// TODO: add an option for having synced saving, that is when call enqueueSave, that will instead,
// instantly
...
public class SectionSavingService {
  ...
  private void processJob() {
    ...
    try {
      // Unmark it dirty here (if it wasnt or w/e) so that it doesnt pointlessly resave (in theory
      ...
      Logger.error(
          "Voxy saver had an exception while executing please check logs and report error", e);
    }
    section.release();
  }
  ...
  public boolean enqueueSave(
      WorldEngine in, WorldSection section, boolean nonBlocking, boolean sectionAlreadyAcquired) {
    ...
    if (section.exchangeIsInSaveQueue(true)) {
      ...
      if ((!nonBlocking) && this.getTaskCount() > SOFT_MAX_QUEUE_SIZE) {
        ...
        // If we are still full, process entries in the queue ourselves instead of waiting for the
        // service
  ...
  public void shutdown() {
    if (this.service.numJobs() != 0) {
      Logger.error(
          "Voxy section saving still in progress, estimated "
              + this.service.numJobs()
              + " sections remaining.");
