...
// allows the pooling of multiple threadpools together while prioritizing the work the original was
// ment for
public class MultiThreadPrioritySemaphore {
  public static final class Block extends TrackedObject {
    ...
    // private final AtomicInteger debt = new AtomicInteger();//the debt of the work pool semphore
    ...
    public void acquire(
        boolean
            runJob) { // Block until a permit for this block is availbe, other jobs maybe executed
      // while we wait
      /*
      while (true) {
      ...
      // Absolutly no idea if this shitty thing functions correctly... at all, it very much probably
      // doesnt
      while (true) {
        if (runJob) {
          this.blockSemaphore.acquireUninterruptibly(); // Block on all
  ...
  private boolean tryRun(Block block) {
    ...
    while (true) {
      ...
      if (status == 1)
        return false; // we didnt run a job because there either wasnt any or no services exist
      if (2 <= status) { // 2 and 3 mean failed to find a service that can currently run, but should
        // try again after a delay
        try {
          if (block.localSemaphore.tryAcquire(
              10, TimeUnit.MILLISECONDS)) { // Await 10 millis for a local job to come in
