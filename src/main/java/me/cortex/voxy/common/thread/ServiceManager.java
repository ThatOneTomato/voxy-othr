...
public class ServiceManager {
  ...
  private final ThreadLocal<ThreadCtx> accelerationContext =
      ThreadLocal.withInitial(ThreadCtx::new);
  ...
  public Service createService(
      Supplier<Pair<Runnable, Runnable>> ctxFactory, long weight, String name) {
    return this.createService(ctxFactory, weight, name, null);
  }

  public synchronized Service createService(
      Supplier<Pair<Runnable, Runnable>> ctxFactory,
      long weight,
      String name,
  ...
  private int runAJob0() { // Executes a single job on the current thread
    ...
    while (true) {
      ...
      int shiftFactor =
          (ctx.shiftFactor++)
              & Integer
                  .MAX_VALUE; // We cycle and shift the starting service when choosing to prevent
      // bias
      int c = shiftFactor;
      ...
      for (int i = 0; i < services.length; i++) {
        ...
        if (service.limiter != null
            && (((skipMsk & (1L << i)) != 0) || !service.limiter.getAsBoolean())) {
          skipMsk |= 1L << i;
          continue;
        }
        ...
        for (var s : this.services) {
          if (s.isLive()) {
            throw new IllegalStateException(
                "Service '" + s.name + "' was not in shutdown when manager shutdown");
          }
        }
      }
    }
