...
public class MemoryBuffer extends TrackedObject {
  private static final boolean TRACK_MEMORY_BUFFERS =
      VoxyFlags.isVerificationFlagOn("trackBuffers");
  ...
  // Creates a new MemoryBuffer, defunking this buffer and sets the size to be a subsize of the
  // current size
  public MemoryBuffer subSize(long size) {
    if (size > this.size || size <= 0) {
      throw new IllegalArgumentException(
          "Requested size larger than current size, or less than 0, requested: "
              + size
              + " capacity: "
              + this.size);
    }
  ...
  // methods for get and set, that way can have a single unifed system to ensure memory access
  // bounds
