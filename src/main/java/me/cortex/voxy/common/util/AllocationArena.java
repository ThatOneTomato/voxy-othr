...
// FIXME: NOTE: if there is a free block of size > 2^30 EVERYTHING BREAKS, need to either increase
// size
...
// TODO: replace the LongAVLTreeSet with a custom implementation that doesnt cause allocations when
// searching
// and see if something like a RBTree is any better
public class AllocationArena {
  public static final long SIZE_LIMIT = -1;

  private static final int ADDR_BITS =
      34; // This gives max size per allocation of 2^30 and max address of 2^39
  ...
  public int getLargestFreeBlockSize(int index) {
    ...
    for (; index > 0 && iter.hasPrevious(); index--) {
      iter.previousLong();
    }
  ...
  public long alloc(int size) { // TODO: add alignment support
    ...
    if (!iter.hasNext()) { // No free space for allocation
      ...
      iter.remove();
      if ((slot >>> ADDR_BITS)
          == size) { // If the allocation and slot is the same size, just add it to the taken
  ...
  public int free(long addr) { // Returns size of freed memory
    ...
    // Note: if there is a previous entry, it means that it is guaranteed for the ending address to
    // either
    ...
    } // If there is no previous it means were at the start of the buffer, we might need to merge
    // with block 0 if we are not block 0
    else if (!this.FREE
        .isEmpty()) { // if free is not empty it means we must merge with block of free starting at
      // 0
      // if (addr != 0)//FIXME: this is very dodgy solution, if addr == 0 it means its impossible
      // for there to be a previous element
      if (this.FREE.remove(
          addr << ADDR_BITS)) { // Attempt to remove block 0, this is very dodgy as it assumes block
        // zero is 0 addr n size
        slot = addr + size; // slot at address 0 and size of 0 block + new block
      }
    }

    // If there is a next element it is guarenteed to either be the next block, or indicate that
    // there is
    ...
    if (iter.hasNext()) {
      ...
      if (endAddr
          != nextSlot >>> SIZE_BITS) { // It means there is a memory block to be merged in FREE
    ...
    } // if there is no next block it means that we have reached the end of the allocation sections
    // and we can shrink the buffer
    else {
      this.resized = true;
  ...
  public boolean expand(long addr, int extra) {
    ...
    if (iter.hasNext()) {
      ...
      if (extra <= delta) {
        ...
        // else There is exactly enough free space, so removing the free block and updating the
      ...
      if (this.totalSize + extra
          > this.sizeLimit) // If expanding and we would exceed the size limit, dont resize
      return false;
      iter.remove();
