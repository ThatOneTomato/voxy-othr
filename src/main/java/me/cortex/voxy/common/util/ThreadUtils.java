...
public class ThreadUtils {
  ...
  static {
    if (isWindows) {
      ...
      SetThreadSelectedCpuSetMasks =
          Kernel32.getLibrary().getFunctionAddress("SetThreadSelectedCpuSetMasks");
    } else {
      SetThreadPriority = 0;
  ...
  public static boolean SetThreadSelectedCpuSetMasksWin32(long[] masks, short[] groups) {
    ...
    if (masks == null) {
      int retVal =
          JNI.invokePPCI(Kernel32.GetCurrentThread(), 0, (short) 0, SetThreadSelectedCpuSetMasks);
      if (retVal == 0) {
    ...
    try (var stack = MemoryStack.stackPush()) {
      ...
      int retVal =
          JNI.invokePPCI(
              Kernel32.GetCurrentThread(), ptr, (short) masks.length, SetThreadSelectedCpuSetMasks);
      if (retVal == 0) {
        throw new IllegalStateException();
      }
      return true;
