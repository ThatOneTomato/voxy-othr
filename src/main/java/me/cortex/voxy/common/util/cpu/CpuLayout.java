...
public class CpuLayout {
  ...
  public static void setThreadAffinity(Affinity... affinities) {
    ...
    if (platform == Util.OS.WINDOWS) {
      ...
      ThreadUtils.SetThreadSelectedCpuSetMasksWin32(
          Arrays.copyOf(msks, i), Arrays.copyOf(groups, i));
  ...
  private static Core[] generateCoreLayoutWindows() {
    var cores =
        Kernel32Util.getLogicalProcessorInformationEx(
  ...
  private static Core[] generateCoreLayoutLinux() {
    ...
    for (var thread : processor.getLogicalProcessors()) {
      var aff =
          affinityMsk.getOrDefault(
              thread.getPhysicalProcessorNumber(),
      ...
      affinityMsk.put(
          thread.getPhysicalProcessorNumber(),
          new Affinity(
              aff.msk | (1L << thread.getProcessorNumber()), (short) thread.getProcessorGroup()));
    }
