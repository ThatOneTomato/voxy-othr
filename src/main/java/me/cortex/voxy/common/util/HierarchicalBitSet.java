...
public class HierarchicalBitSet {
  ...
  public int allocateNextConsecutiveCounted(int count) {
    if (count > 64) {
      throw new IllegalStateException(
          "Count to large for current implementation which has fastpath");
    }
    if (this.A == -1) {
      return -1;
    }
    ...
    while (true) {
      ...
      if ((fusedValue & chkMsk) != 0) {
        ...
        i +=
            Long.numberOfTrailingZeros(
                fusedValue); // Skip as much as possible (i.e. skip to the next 1 bit)
      ...
      // (can  do it by first setting/updating the lower D index and propagating, then the upper D
      // index (if it has/needs one))
