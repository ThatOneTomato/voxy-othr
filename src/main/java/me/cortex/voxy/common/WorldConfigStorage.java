                ...
                new TypeAdapter<InnerHolder<T>>() {
                  ...
                  public void write(JsonWriter writer, InnerHolder<T> obj) throws IOException {
                    ...
                    for (var entry : obj.worldConfigs.entrySet()) {
                      ...
                      if (entry.getValue() != null) {
                        WorldConfigStorage.this
                            .gson
                            .getAdapter(WorldConfigStorage.this.configType)
                  ...
                  public InnerHolder<T> read(JsonReader in) throws IOException {
                    ...
                    if (ver.getAsInt() != FORMAT_VERSION) {
                      Logger.error(
                          "Trying to load config from non matching version, got: "
                              + ver.getAsInt()
                              + " expect "
                              + FORMAT_VERSION);
                      return null;
                      ...
                      if (!valTree.isJsonNull()) {
                        val =
                            WorldConfigStorage.this
                                .gson
                                .getAdapter(WorldConfigStorage.this.configType)
                                .fromJsonTree(valTree);
                      }

                      if (holder.worldConfigs.containsValue(key)) {
                        Logger.error(
                            "World config contained duplicate worldId keys: "
                                + key
                                + " overriding config");
                      }
