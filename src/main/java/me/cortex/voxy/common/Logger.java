...
public class Logger {
  ...
  public static void error(Object... args) {
    ...
    String error =
        (INSERT_CLASS ? ("[" + callClsName() + "]: ") : "")
            + Stream.of(args).map(Logger::objToString).collect(Collectors.joining(" "));
    LOGGER.error(error, throwable);
    if (PlatformAccess.IS_IN_MINECRAFT && !PlatformAccess.IS_DEDICATED_SERVER) {
      showInHUD(error); // This is done so that on dedicated server, the Minecraft client class isnt
      // loaded
    }
  }
