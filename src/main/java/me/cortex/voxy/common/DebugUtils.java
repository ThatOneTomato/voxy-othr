...
public class DebugUtils {
        ...
        new Thread(
            () -> {
              ...
              try {
                ...
                Logger.showInHUD(
                    "Starting tln child verification"
                        + (attemptRepair ? " attemptting repairs on error" : ""));
  ...
  public static void verifyTopNodeChildren(
      WorldEngine world, int X, int Y, int Z, boolean tryRepair) {
    // TODO: can speed this up if needed by not getting the children and instead caching the
    // previous getNonEmptyChildren result
                      ...
                      "Section empty child mask not correct "
                          ...
                          + String.format(
                                  ...
                                  Integer.toBinaryString(
                                      Byte.toUnsignedInt(own.getNonEmptyChildren())))
                              .replace(' ', '0')
                          + " expected: "
                          + String.format("%8s", Integer.toBinaryString(Byte.toUnsignedInt(msk)))
                              .replace(' ', '0'));
                  if (tryRepair) {
                    for (int child = 0; child < 8; child++) {
                      var section =
