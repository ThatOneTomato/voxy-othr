...
import me.cortex.voxy.impl.mixin.sable.SableSubLevelHoldingChunkMapAccessor;
...
/**
 * Keeps the chunks that contain (or hold serialized) Sable sub levels loaded while any player is
 * within the simulated contraption render distance, so distant contraptions keep being simulated
 * and can be rendered against Voxy's LODs.
 */
public final class SableLodChunkManager {
  private static final TicketType<ChunkPos> VOXY_SABLE_LOD_TICKET =
      TicketType.create("voxy_sable_lod", Comparator.comparingLong(ChunkPos::toLong));
  ...
  public static void updateTickets(
      ServerLevel level, LongSet trackedChunks, LongSet trackedHoldingChunks) {
    if (sableUnavailable) {
    ...
    try {
      ...
      double maxHorizontalDistanceSquared =
          horizontalRenderDistanceBlocks * horizontalRenderDistanceBlocks;
      ...
      for (ServerSubLevel subLevel : container.getAllSubLevels()) {
        ...
        if (bounds == null
            || !isWithinHorizontalDistance(level, bounds, maxHorizontalDistanceSquared)) {
          continue;
        }
      ...
      updateHoldingChunkLoads(
          level,
          container.getHoldingChunkMap(),
          desiredChunks,
          desiredHoldingChunks,
          trackedHoldingChunks,
          maxHorizontalDistanceSquared);
  ...
  public static void clearTickets(ServerLevel level, LongSet trackedChunks) {
    ...
    while (iterator.hasNext()) {
      ...
      level
          .getChunkSource()
          .removeRegionTicket(VOXY_SABLE_LOD_TICKET, chunkPos, TICKET_DISTANCE, chunkPos);
      iterator.remove();
    }
  }
  ...
  public static boolean shouldTreatChunkAsLoaded(ServerLevel level, int chunkX, int chunkZ) {
    ...
    return isChunkWithinHorizontalDistance(
        level,
  ...
  private static void updateHoldingChunkLoads(
    ...
    Long2ObjectMap<SubLevelHoldingChunk> loadedHoldingChunks =
        ((SableSubLevelHoldingChunkMapAccessor) holdingChunkMap).voxy$getLoadedHoldingChunks();
    ...
    while (iterator.hasNext()) {
      ...
      SubLevelHoldingChunk holdingChunk =
          loadedHoldingChunks == null ? null : loadedHoldingChunks.get(chunk);
    ...
    removeStaleHoldingChunkLoads(
        level, holdingChunkMap, trackedHoldingChunks, desiredHoldingChunks);
  }

  private static LongSet getKnownHoldingChunks(
      ServerLevel level, SubLevelHoldingChunkMap holdingChunkMap) {
  ...
  private static void removeStaleHoldingChunkLoads(
      ServerLevel level,
      SubLevelHoldingChunkMap holdingChunkMap,
      LongSet trackedHoldingChunks,
      LongSet desiredHoldingChunks) {
  ...
  private static void clearHoldingChunkLoads(ServerLevel level, LongSet trackedHoldingChunks) {
    ...
    try {
      ServerSubLevelContainer container = SubLevelContainer.getContainer(level);
      SubLevelHoldingChunkMap holdingChunkMap =
          container == null ? null : container.getHoldingChunkMap();
      if (holdingChunkMap != null) {
        LongIterator iterator = trackedHoldingChunks.iterator();
        while (iterator.hasNext()) {
  ...
  private static void addMissingTickets(
      ServerLevel level, LongSet trackedChunks, LongSet desiredChunks) {
    ...
    while (iterator.hasNext()) {
      ...
      if (trackedChunks.add(chunk)) {
        ...
        level
            .getChunkSource()
            .addRegionTicket(VOXY_SABLE_LOD_TICKET, chunkPos, TICKET_DISTANCE, chunkPos);
      }
    }
  }

  private static void removeStaleTickets(
      ServerLevel level, LongSet trackedChunks, LongSet desiredChunks) {
    LongIterator iterator = trackedChunks.iterator();
    while (iterator.hasNext()) {
      long chunk = iterator.nextLong();
  ...
  private static void addChunkBounds(
      ServerLevel level,
  ...
  private static boolean isWithinHorizontalDistance(
      ServerLevel level, BoundingBox3dc bounds, double maxHorizontalDistanceSquared) {
  ...
  private static boolean isChunkWithinHorizontalDistance(
      ServerLevel level, ChunkPos chunkPos, double maxHorizontalDistanceSquared) {
