package me.cortex.voxy.impl.compat.sable;
...
/**
 * Computes the "simulated contraption render distance" used by the Sable compatibility layer: the
 * horizontal range (in blocks) within which Sable sub levels (contraptions) should stay
 * loaded/tracked, interpolated between the vanilla render distance and Voxy's LOD render distance.
 *
 * <p>On the integrated server the client pushes its live config via {@link
 * #updateClientConfig(boolean, double, int)}; as a fallback (and on first tick) the voxy config
 * json is read directly. On dedicated servers there is no Voxy client config, so Sable's own {@code
 * sub_level_tracking_range} setting is used instead.
 */
public final class SableContraptionRenderDistance {
  private static final Path CONFIG_PATH =
      PlatformAccess.get().getConfigDir().resolve("voxy-config.json");
  private static final Path SABLE_COMMON_CONFIG_PATH =
      PlatformAccess.get().getConfigDir().resolve("sable-common.toml");
  ...
  private static final ConfigSnapshot DISABLED_CONFIG =
      new ConfigSnapshot(false, 0.0, DEFAULT_PERCENT, Long.MIN_VALUE);
  ...
  public static double getRangeBlocks(ServerLevel level) {
    ...
    int contraptionDistanceChunks =
        extendVanillaRenderDistanceChunks(
  ...
  public static int extendVanillaRenderDistanceChunks(
    ...
    int voxyRenderDistanceChunks =
        (int) Math.ceil(sectionRenderDistance * CHUNKS_PER_SECTION_RENDER_DISTANCE);
        ...
        (int)
            Math.ceil(
                vanillaDistanceChunks
                    + ((voxyRenderDistanceChunks - vanillaDistanceChunks) * (percent / 100.0D))));
  }
  ...
  public static void updateClientConfig(
    ...
    if (!enabled || sectionRenderDistance <= 0.0) {
      runtimeClientConfig =
          new ConfigSnapshot(false, 0.0, simulatedContraptionRenderDistancePercent, Long.MAX_VALUE);
      return;
    }

    runtimeClientConfig =
        new ConfigSnapshot(
            true, sectionRenderDistance, simulatedContraptionRenderDistancePercent, Long.MAX_VALUE);
  }
  ...
  private static double getDedicatedServerRangeBlocks(long gameTime) {
    ...
    try {
      lastModified =
          Files.exists(SABLE_COMMON_CONFIG_PATH)
              ? Files.getLastModifiedTime(SABLE_COMMON_CONFIG_PATH).toMillis()
              : Long.MIN_VALUE;
    } catch (IOException e) {
  ...
  private static double loadDedicatedServerRangeBlocks() {
    ...
    try (BufferedReader reader = Files.newBufferedReader(SABLE_COMMON_CONFIG_PATH)) {
      ...
      Logger.error(
          "Failed to load Sable sub_level_tracking_range for Voxy Sable LOD compatibility", e);
    }
    ...
    try {
      lastModified =
          Files.exists(CONFIG_PATH)
              ? Files.getLastModifiedTime(CONFIG_PATH).toMillis()
              : Long.MIN_VALUE;
    } catch (IOException e) {
  ...
  private static ConfigSnapshot loadConfig(long lastModified) {
    if (!Files.exists(CONFIG_PATH)) {
      return new ConfigSnapshot(
          true, DEFAULT_SECTION_RENDER_DISTANCE, DEFAULT_PERCENT, lastModified);
    }

    try (Reader reader = Files.newBufferedReader(CONFIG_PATH)) {
      JsonObject root = JsonParser.parseReader(reader).getAsJsonObject();
      boolean enabled =
          getBoolean(root, "enabled", true) && getBoolean(root, "enable_rendering", true);
      double sectionRenderDistance =
          getDouble(root, "section_render_distance", DEFAULT_SECTION_RENDER_DISTANCE);
      int simulatedContraptionPercent =
          getInt(root, "simulated_contraption_render_distance_percent", DEFAULT_PERCENT);
      ...
      return new ConfigSnapshot(
          true, sectionRenderDistance, simulatedContraptionPercent, lastModified);
    } catch (Exception e) {
