...
/**
 * Caches sky light data from incoming chunk packets so Sable sub levels can be lit correctly even
 * when their parent chunks are outside the vanilla chunk storage range (only Voxy LODs remain).
 */
public final class SableClientSkyLightCache {
  ...
  // The cache is only useful for Sable sub level lighting; skip all packet processing when
  // Sable isn't installed.
  private static boolean unavailable = !PlatformAccess.get().isModLoaded("sable");

  private SableClientSkyLightCache() {}

  public static synchronized void cacheFromPacket(
      ClientLevel level, ClientboundLevelChunkWithLightPacket packet) {
    if (unavailable) {
      return;
    }
  ...
  public static synchronized void tick(ClientLevel level) {
    try {
      ...
      Iterator<Map.Entry<Long, CachedSkyLight>> iterator =
          state.skyLightSections.entrySet().iterator();
  ...
  private record PacketLightData(DataLayer[] skyLight) {
    private static @Nullable PacketLightData from(
        ClientLevel level, ClientboundLightUpdatePacketData lightData) {
    ...
    private static boolean unpackLayer(
        ClientLevel level,
        BitSet dataMask,
        BitSet emptyMask,
        List<byte[]> updates,
