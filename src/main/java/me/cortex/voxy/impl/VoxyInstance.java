...
public abstract class VoxyInstance {
  ...
  // have automatic world cleanup after ~1 minute of inactivity and the reference count equaling
  // zero possibly
  // note, the reference count should be separate from the number of active chunks to prevent many
  // issues
  // a world is no longer active once it has no reference counts and no active chunks associated
  // with it
  ...
  private WorldEngine createWorld(WorldIdentifier identifier) {
    ...
    Logger.info(
        "Creating new world engine: "
            + identifier.getLongHash()
            + "@"
  ...
  public void addDebug(List<String> debug) {
    debug.add(
        "MemoryBuffer, Count/Size (mb): "
            + MemoryBuffer.getCount()
            + "/"
    ...
    debug.add(
        "I/S/AWSC: "
            ...
            + "/["
            + this.activeWorlds.values().stream()
