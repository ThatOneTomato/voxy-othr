...
public class WorldIdentifier {
  private static final ResourceKey<DimensionType> NULL_DIM_KEY =
      ResourceKey.create(
          Registries.DIMENSION_TYPE, ResourceLocation.parse("voxy:null_dimension_id"));
  ...
  public WorldIdentifier(
      @NotNull ResourceKey<Level> key,
      long biomeSeed,
      @Nullable ResourceKey<DimensionType> dimension) {
    if (key == null) {
    ...
    this.hashCode =
        mixStafford13(registryKeyHashCode(key))
            ^ mixStafford13(registryKeyHashCode(dimension))
            ^ mixStafford13(biomeSeed);
  }
  ...
  public static String getWorldId(WorldIdentifier identifier) {
    ...
    try {
      return bytesToHex(MessageDigest.getInstance("SHA-256").digest(data.getBytes()))
          .substring(0, 32);
