...
public class WorldImporter implements DataImporter {
  ...
  private final AtomicInteger estimatedTotalChunks =
      new AtomicInteger(); // Slowly converges to the true value
  ...
  public WorldImporter(
      WorldEngine worldEngine, Level mcWorld, ServiceManager sm, BooleanSupplier runChecker) {
    ...
    this.defaultBiomeProvider =
        new PalettedContainerRO<>() {
          @Override
          ...
          @Override
          public PackedData<Holder<Biome>> pack(
              IdMap<Holder<Biome>> idMap, PalettedContainer.Strategy strategy) {
            return null;
          }
        };
  ...
  public void importZippedRegionDirectoryAsync(File zip, String innerDirectory) {
    try {
      ...
      this.importRegionsAsync(
          regions.toArray(ZipArchiveEntry[]::new),
          (entry) -> {
            if (entry.getSize() == 0) {
              return;
            }
            ...
            try {
              ...
              Logger.error(
                  "Invalid format for region position, x: \""
                      + sections[1]
                      + "\" z: \""
                      + sections[2]
                      + "\" skipping region");
        ...
        new Thread(
            () -> {
              ...
              for (var file : regionFiles) {
                ...
                while ((this.totalChunks.get() - this.chunksProcessed.get() > 10_000)
                    && this.isRunning) {
                  try {
                    Thread.sleep(1);
                  } catch (InterruptedException e) {
    ...
    for (int idx = 0; idx < 1024; idx++) {
      int sectorMeta =
          Integer.reverseBytes(
              MemoryUtil.memGetInt(regionFile.address + idx * 4)); // Assumes little endian
      if (sectorMeta == 0) {
        // Empty chunk
        continue;
      }
...

      {
        ...
        if (m == 0) {
          ...
          if (regionFile.size < (n + sectorStart * 4096L)) {
            ...
            Logger.error(
                "Chunk stream is truncated: expected " + n + " but read " + (chunkLen - 5));
          } else if (n < 0) {
  ...
  private void importChunkNBT(CompoundTag chunk, int regionX, int regionZ) {
    ...
    if (status != ChunkStatus.FULL
        && status != ChunkStatus.EMPTY) { // We also import empty since they are from data upgrade
      this.totalChunks.decrementAndGet();
      return;
    }
    ...
    this.updateCallback.onUpdate(
        this.chunksProcessed.incrementAndGet(), this.estimatedTotalChunks.get());
  }
  ...
  private void importSectionNBT(int x, int y, int z, CompoundTag section) {
    ...
    var blockStatesRes =
        blockStateCodec.parse(NbtOps.INSTANCE, section.getCompound("block_states"));
    if (!blockStatesRes.hasResultOrPartial()) {
      // TODO: if its only partial, it means should try to upgrade the nbt format with
      // datafixerupper probably
      return;
    }
    ...
    if (!optBiomes.isEmpty()) {
      biomes =
          this.biomeCodec
              .parse(NbtOps.INSTANCE, optBiomes)
              .result()
